...
| **P14 a→l** dette, code mort, index, énumérations, migrations liées | FAIBLE | ✅ **corrigé** (vague 7) | a/c/d/f/h déjà clos en vagues 1-2 et 6 ; k/l migration rejouable et garantie ; j 10 enums sur 14 colonnes, avec `NPS` et `OUI_NON` que les commentaires du schéma oubliaient ; g prompt et budgets de tokens uniques ; i code mort et `SESSION_SECRET` supprimés ; `deploy/` désindexé et `DEPLOIEMENT.md` corrigé (il pointait vers un dossier inexistant). **Restent ouverts** : les ~8 jeux de valeurs non contraints en base, et les index recommandés au §2.2 que je n'ai pas lus. |
