...
| i | `UserSession` + `SESSION_SECRET` déclarés, validés, jamais lus ; `StatistiquesMensuelles` et `Reponse.audio_url` morts ; `logo_dark_url` lu côté client sans colonne | ✅ **corrigé** (vague 7) — modèle, table, colonne et secret supprimés ; `SESSION_SECRET` n'est plus exigé au démarrage |
| j | Aucun `enum` en base : ~20 jeux de valeurs sont des `String` libres (`type_reponse`, `scoring_mode`, `status`, …) | ✅ **corrigé** (vagues 7-8) — 19 enums sur 24 colonnes ; seconde vague : `MIGRATED` est une vraie 3e provenance (backfill), `NPS`/`OUI_NON`/`EN_COURS` que les commentaires oubliaient. Ne restent libres que les libellés ouverts (`type_guichet`, noms) et les champs techniques (clés, JWT). |
...
| **P14 a→l** dette, code mort, index, énumérations, migrations liées | FAIBLE | ✅ **corrigé** (vague 7) | a/c/d/f/h déjà clos ; k/l migration rejouable ; j 19 enums, gardes applicatives adossées aux types générés ; g prompt et budgets uniques ; i code mort et `SESSION_SECRET` supprimés ; `deploy/` désindexé, doc de déploiement corrigé ; 7 index d'agrégats/tenant posés. **Reste ouvert** : les index du §2.2 hors périmètre agrégats/tenant — aucun autre recommandé par l'audit. |
...
| Index | 7 index posés en vague 8 (`20260927001200`) : les 4 `Reponse.*` demandés et les 3 `id_entreprise` (User, Critere, Service). Noms identiques à ceux de Prisma, `IF NOT EXISTS`, migration rejouable. Dette soldée. | scans sur les agrégats par critère/service et sur les listings par tenant |
| Code mort | 6 fonctions + 2 modèles (`StatistiquesMensuelles`, `UserSession`) + 1 champ (`audio_url`) + 1 branche UI (`logo_dark_url`) | surface de code réduite ; **correction de l'audit** : `Logs` est cité comme mort alors que `src/server/notifications/gateway.ts` le référence (3 occurrences) — le document avait tort, pas le code. `logo_dark_url` a 2 références, donc la branche UI est bien morte mais le champ ne l'est pas |
