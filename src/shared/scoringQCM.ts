...
export function parseOptionsCSV(brut: string | null | undefined): string[] {
  return String(brut || '')
    .split(',')
    .map((o) => o.trim())
    .filter(Boolean);
...
export function parseScoresCSV(brut: string | null | undefined): number[] | null {
  if (!brut || !brut.trim()) return null;
  const scores = String(brut)
    .split(',')
    .map((s) => Number(s.trim()));
  if (scores.some((s) => !Number.isInteger(s) || s < 1 || s > 5)) return null;
  return scores;
}
...
const echapperRegex = (s: string): string =>
  s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

const cacheMotifs = new Map<string, RegExp>();

function motifEntree(entree: string): RegExp {
...
const contientUn = (texte: string, entrees: string[]): string | null =>
  entrees.find((e) => motifEntree(e).test(texte)) ?? null;
 ...
 */
export function infererScoreOption(option: string): number | null {
...
export function infererScoresOptions(options: string[]): (number | null)[] {
  return options.map(infererScoreOption);
}

// ── Mapping effectif d'un critère ──────────────────────────────────────────
 ...
 */
export function scoresEffectifsPourCritere(
  ...
  if (stockes && stockes.length === options.length) return stockes;
...
function indexOptionParLabel(options: string[], label: string): number {
 ...
 */
export function validerScoresExplicites(
  optionsBrut: string,
  scoresBrut: string | null | undefined
): string {
  ...
  if (scores.length !== options.length) {
    throw new Error(
      `Scores invalides : ${scores.length} score(s) pour ${options.length} choix — il en faut exactement un par choix.`
    );
  }
  return scores.join(',');
}
 ...
 */
export function construireScoresAStocker(
  optionsBrut: string,
  scoresBrut?: string | null
): string | null {
  const options = parseOptionsCSV(optionsBrut);
  if (options.length === 0) return null;
  if (scoresBrut && scoresBrut.trim()) return validerScoresExplicites(optionsBrut, scoresBrut);
  const inferes = infererScoresOptions(options);
  if (inferes.some((s) => s === null)) return null;
  return (inferes as number[]).join(',');
}
