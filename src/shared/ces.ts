...
export const ECHELLES_CES: EchelleCES[] = [5, 7];
 ...
 */
export const BANDES_CES: Record<EchelleCES, BandeCESDef[]> = {
  5: [
...
export const LIBELLES_ECHELLE_CES: Record<EchelleCES, string[]> = {
...
export function estEchelleCES(v: number): v is EchelleCES {
  return v === 5 || v === 7;
}

/** Bande d'une note d'effort brute. Hors bornes → null (jamais de bande inventée). */
export function bandeCES(note: number, echelle: EchelleCES): BandeCES | null {
 ...
 */
export function topBoxCES(note: number, echelle: EchelleCES): boolean {
 ...
 */
export function scoreEffort100(note: number, echelle: EchelleCES): number | null {
...
export function agregerCES(notes: number[], echelle: EchelleCES): AgregationCES {
  ...
  for (const note of notes) {
    if (!Number.isInteger(note) || note < 1 || note > echelle) continue; // ignorée, pas comptée
