...
import {
  ...
  renvoyerInvitation, changerLimitesEntreprise,
} from 'wasp/client/operations'
import {
  ArrowLeft, Building2, Users, MapPin, MessageSquare, AlertTriangle,
...
export default function CompanyDetailsPage({ id }: { id: number }) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {message && (
        <div className="flex items-center gap-2 rounded-2xl border border-success/30 bg-success/10 p-4 text-sm font-bold text-success">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {tiles.map(({ label, value, limite, icon: Icon }) => {
          ...
          return (
            <div key={label} className="rounded-2xl border border-border/80 bg-card p-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <section className="rounded-2xl border border-border/80 bg-card p-6">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            {e.invitation_active && (
              <div className="flex items-center gap-2 rounded-xl bg-info/10 p-3 text-xs font-bold text-info">
          ...
          <div className="w-full max-w-md rounded-2xl bg-card p-6 shadow-xl">
              ...
              {([['agences', 'Agences'], ['utilisateurs', 'Utilisateurs'], ['guichets', 'Guichets']] as const).map(([key, label]) => (
                <div key={key}>
                  ...
                  <input
                    ...
                    min={1}
                    value={limites[key]}
                    onChange={(ev) => setLimites((l) => ({ ...l, [key]: Number(ev.target.value) }))}
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button
                onClick={async () => {
                  try {
                    const planEl = document.getElementById('select-plan') as HTMLSelectElement | null
