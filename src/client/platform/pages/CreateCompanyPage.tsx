    ...
    return (
      <div className="mx-auto max-w-xl py-10 text-center">
        ...
        <div className="mt-8 flex justify-center gap-3">
          <Link
            to={`/platform/entreprises/${succes.id_entreprise}`}
          ...
          <button
            onClick={() => { setSucces(null); setEtape(0); setForm((f) => ({ ...f, nom_entreprise: '', nom_court: '', admin_prenom: '', admin_nom: '' })) }}
            className="inline-flex items-center gap-2 rounded-xl border border-border px-5 py-3 text-sm font-bold text-foreground hover:bg-muted"
          >
      ...
      <ol className="flex items-center gap-2" aria-label="Progression du wizard">
        {ETAPES.map((label, i) => (
          <li key={label} className="flex flex-1 items-center gap-2">
            <span className={`flex size-7 shrink-0 items-center justify-center rounded-full text-xs font-black ${
              i < etape ? 'bg-success text-white' : i === etape ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'
            }`}>
              {i < etape ? '✓' : `0${i + 1}`}
            </span>
      ...
      <div className="rounded-2xl border border-border/80 bg-card p-6 md:p-8">
        ...
        {etape === 1 && (
          <section className="space-y-4">
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="grid gap-2">
                {[
                  ...
                  <label key={m.id} className={`flex cursor-pointer items-start gap-3 rounded-xl border-2 p-3 transition-colors ${form.mode === m.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/40'}`}>
                    <input
                      type="radio"
                      name="mode-pilotage"
                      value={m.id}
                      ...
                      onChange={(e) => {
                        ...
                        set('mode', v);
                        if (v === 'CHEF_MONO') set('limite_agences', 1);
                        if (v === 'DIRECTION_RESEAU') setForm((f) => ({ ...f, limite_agences: planCourant.agences }));
                      }}
                      className="mt-1"
                    />
                    <span>
            ...
            {avecAgence && (
              <div className="grid gap-4 sm:grid-cols-2 rounded-xl bg-muted/50 p-4">
                <div>
              ...
              {PLANS.map((p) => {
                ...
                return (
                  <button
                    key={p.id}
                    type="button"
                    onClick={() => setForm((f) => ({ ...f, plan: p.id, limite_agences: p.agences, limite_utilisateurs: p.utilisateurs, limite_guichets: p.guichets }))}
                    className={`rounded-2xl border-2 p-4 text-left transition-colors ${
                      actif ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/40'
                    }`}
                  >
                    ...
                    <ul className="mt-2 space-y-1">
                      {p.features.map((f) => (
                        <li key={f} className="text-[11px] font-semibold text-muted-foreground">• {f}</li>
                      ))}
                    </ul>
                  </button>
                )
        ...
        {etape === 3 && (
          <section className="space-y-4">
            ...
            <dl className="space-y-2 rounded-2xl bg-muted/50 p-5 text-sm">
              ...
              {avecAgence && (
                <div className="flex justify-between gap-3"><dt className="text-muted-foreground">1ère agence</dt><dd className="text-right font-bold text-foreground">{form.agence_nom} — {form.agence_commune}</dd></div>
              )}
            ...
            <ul className="space-y-1.5 text-sm text-muted-foreground">
              {['Compte entreprise (statut ACTIVE)', 'Compte administrateur (DIRECTION)', 'Invitation sécurisée par email (24 h)', 'Action journalisée (AuditLog)'].map((x) => (
        ...
        <div className="mt-8 flex items-center justify-between border-t border-border/60 pt-5">
          {etape > 0 ? (
            <button onClick={() => setEtape(etape - 1)} className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold text-muted-foreground hover:bg-muted">
          ...
          {etape < 3 ? (
            <button
              ...
              disabled={(etape === 0 && !etape1Ok) || (etape === 1 && !etape2Ok)}
