...
const borne = (n: number) => Math.min(5, Math.max(1, Math.round(n)));

export const visuelPourNote = (score: number) => NOTE_CONFIG[borne(score) - 1];

/** Pastille couleur associée à la note (badges, bordures). */
export const classeCouleurNote = (score: number): string => {
  const n = borne(score);
  if (n <= 2) return 'bg-destructive/10 text-destructive border-destructive/25';
  if (n === 3) return 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/25';
  return 'bg-success/10 text-success border-success/25';
};

/** Barre de progression X/5 (lecture des avis, récapitulatifs). */
export const BarreNote = ({ score, max = 5 }: { score: number; max?: number }) => {
...
export const GrandVisuelNote = ({ score, max = 5 }: { score: number; max?: number }) => {
