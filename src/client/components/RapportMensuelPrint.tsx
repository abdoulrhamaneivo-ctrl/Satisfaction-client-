...
function BarreNote({ note, max = 5 }: { note: number; max?: number }) {
...
function Indicateur({ label, valeur, detail }: { label: string; valeur: string; detail?: string }) {
  return (
    <div style={{ border: '1px solid #E9ECEF', borderRadius: 10, padding: '14px 12px', textAlign: 'center' }}>
...
const STATUT_STYLES: Record<string, { bg: string; fg: string; label: string }> = {
...
export const RapportMensuelPrint = React.forwardRef<HTMLDivElement, RapportProps>((props, ref) => {
  ...
  const tachesEnRetard = taches.filter((t: any) => {
    if (!t.date_echeance || t.statut_tache === 'TERMINEE') return false;
    return new Date(t.date_echeance) < new Date();
  }).length;

  const fmtDate = (d: Date) =>
    d.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' });
  const fmtDateCourt = (d: string | Date) =>
    new Date(d).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: '2-digit' });

  const fleche = (v: number) => (v > 0 ? '▲' : v < 0 ? '▼' : '—');
  const signe = (v: number, unite = '') =>
    `${v > 0 ? '+' : ''}${Number.isInteger(v) ? v : v.toFixed(1)}${unite}`;

  const alertesRapport = alertes.slice(0, 10);
  const tachesParAlerte = new Map<any, TacheRapport>();
  for (const t of taches) {
    if (t.id_alerte != null && !tachesParAlerte.has(t.id_alerte)) tachesParAlerte.set(t.id_alerte, t);
  }
  ...
  return (
    ...
    >
      ...
      <section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginTop: 10 }}>
          <Indicateur
            ...
            valeur={`${alertesCloturees}/${alertes.length}`}
            detail="Suivi jusqu'à résolution"
          />
          <Indicateur
            label="Actions correctives"
            valeur={String(taches.length)}
            detail={tachesEnRetard > 0 ? `dont ${tachesEnRetard} en retard` : 'aucune en retard'}
          />
          <Indicateur
      ...
      <section>
        ...
        {totalAvis === 0 ? (
          <p style={{ fontSize: 11, color: '#6C757D', fontStyle: 'italic' }}>
            Aucun avis collecté sur cette période — les indicateurs seront renseignés dès les premières soumissions.
          </p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {distribution.map((d) => (
              <div key={d.note} style={{ display: 'grid', gridTemplateColumns: '52px 1fr 90px', alignItems: 'center', gap: 10 }}>
      ...
      <section>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {radarData.map((r) => (
            <div key={r.subject} style={{ display: 'grid', gridTemplateColumns: '170px 1fr 52px', alignItems: 'center', gap: 10 }}>
        ...
        <section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            {themes.slice(0, 6).map((t) => (
              <div key={t.theme} style={{ display: 'grid', gridTemplateColumns: '150px 1fr 40px', alignItems: 'center', gap: 10 }}>
      ...
      {guichetsFaibles.length > 0 && (
        <section>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
            ...
            <tbody>
              {guichetsFaibles.map((g) => (
                <tr key={g.nom} style={{ borderBottom: '1px solid #F1F3F5' }}>
      ...
      <section>
        ...
        {alertes.length > 10 ? (
          <p style={{ fontSize: 9.5, color: '#6C757D', marginTop: 6 }}>
