    ...
    <div className={cn('pointer-events-none absolute inset-0 overflow-hidden', className)} aria-hidden="true">
      {BLOBS.map((blob, i) => (
        <motion.div
          ...
          animate={reduce ? undefined : { x: blob.x, y: blob.y, scale: [1, 1.08, 0.96, 1] }}
          transition={reduce ? undefined : { duration: blob.duration, repeat: Infinity, ease: 'easeInOut', delay: blob.delay }}
        />
      ))}
    </div>
  );
