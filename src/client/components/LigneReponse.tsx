...
export const LigneReponse = ({ r, texteGroupe }: { r: any; texteGroupe?: string | null }) => {
  const type = r.critere?.type_reponse as string | undefined;
  ...
  if (type === 'TEXTE') {
    ...
    if (!texteSpecifique) return null; // déjà cité dans le commentaire global
    return (
      ...
      <li className={`${coquille} flex-col !items-stretch gap-1.5`}>
        ...
        <span className="flex flex-wrap gap-1.5">
          {choix.map((c: string, i: number) => (
            <span key={i} className="rounded-lg border border-primary/25 bg-primary/10 px-2.5 py-1 text-xs font-bold text-primary">
              {c}
            </span>
          ))}
        </span>
