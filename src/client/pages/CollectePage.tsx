                    ...
                    <div className="flex justify-between items-center gap-1 sm:gap-2 pt-3 w-full min-w-0">
                      {SMILEYS.map((s) => (
                        <button
                          ...
                          aria-label={s.label}
                          className={`text-3xl sm:text-4xl p-2 sm:p-3 flex-1 max-w-[72px] min-h-[52px] min-w-[44px] flex justify-center items-center rounded-2xl hover:bg-muted/80 border border-transparent hover:border-border/60 transition-transform hover:scale-110 active:scale-95 ${BTN_BASE}`}
                        >
                          {s.icon}
                        </button>
                      ))}
                  ...
                  {currentCritere.type_reponse === 'OUI_NON' && (
                    <div className="grid grid-cols-2 gap-3 sm:gap-4 pt-2">
                      <button
                        ...
                        className={`bg-success/10 hover:bg-success/20 text-success border border-success/30 font-bold py-5 rounded-2xl text-base sm:text-lg transition-colors flex flex-col items-center justify-center gap-1 shadow-sm min-h-[88px] ${BTN_BASE}`}
                      >
                      ...
                      <button
                        ...
                        className={`bg-destructive/10 hover:bg-destructive/20 text-destructive border border-destructive/30 font-bold py-5 rounded-2xl text-base sm:text-lg transition-colors flex flex-col items-center justify-center gap-1 shadow-sm min-h-[88px] ${BTN_BASE}`}
                      >
                        <span className="text-3xl">👎</span>
                        <span>Non</span>
                      </button>
                    ...
                    <div className="flex flex-col gap-2.5 pt-2">
                      {currentCritere.options_reponse?.split(',').map((option: string, index: number) => (
                        <button
                          ...
                          className={`w-full text-left p-4 border border-border/80 rounded-2xl hover:bg-muted text-foreground text-sm font-bold transition-colors flex items-center gap-3 min-h-[52px] ${BTN_BASE}`}
                        >
                      ...
                      <div className={`grid ${colsClass} gap-2 pt-2 w-full min-w-0`}>
                        {valeurs.map((v) => (
                          <button
                            ...
                            className={`w-full h-12 rounded-2xl border border-border/80 bg-background hover:bg-primary/15 hover:border-primary/50 text-base font-bold text-foreground transition-colors flex items-center justify-center font-satoshi ${BTN_BASE}`}
                          >
                            {v}
                          </button>
                        ))}
