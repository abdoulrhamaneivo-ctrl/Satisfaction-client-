...
import {
  CalendarClock, Store, UserCheck2, Clock, AlertTriangle, Pencil, Trash2, Search,
  ChevronLeft, ChevronRight, Copy, Sparkles, LayoutTemplate, CalendarDays, Check,
} from 'lucide-react';
...
export const PlanningPage = () => {
  ...
  const [onglet, setOnglet] = useState<'jour' | 'semaine'>('jour');
  ...
  const [suggestion, setSuggestion] = useState<{ source: string | null; propositions: any[] } | null>(null);

  // --- Semaine type ---
  const [jourType, setJourType] = useState<number>(new Date().getUTCDay());
  ...
  const { data: affectationsDuJour, refetch: refetchAffectations } = useQuery(
    getAffectationsDuJour,
    { id_agence: userAgenceId || 0, date: dateSelectionnee },
    { enabled: !!userAgenceId }
  );
  const { data: modeles, refetch: refetchModeles } = useQuery(
    getModelesHoraires,
    { id_agence: userAgenceId || 0 },
    { enabled: !!userAgenceId && onglet === 'semaine' }
  );
  ...
  const handleSuggerer = async () => {
    ...
    try {
      const res: any = await suggererPlanning({ id_agence: userAgenceId, date: dateSelectionnee });
      setSuggestion(res);
  ...
  const handleAppliquerSuggestion = async () => {
    if (!userAgenceId || !suggestion?.propositions?.length) return;
    setActionEnCours('appliquer');
    try {
      const res = await appliquerSuggestion({
        id_agence: userAgenceId,
        date: dateSelectionnee,
        lignes: suggestion.propositions.map((p: any) => ({
  ...
  const modelesDuJour = (modeles ?? []).filter((m: any) => m.jour_semaine === jourType);
    ...
    <RequireEnterpriseRole>
      ...
      <AmbientBackground>
        <div className="mx-auto max-w-7xl p-6 lg:p-10 space-y-8">
          ...
          <PageHeader
            icon={CalendarClock}
            eyebrow={onglet === 'jour' ? 'Affectations' : 'Semaine type récurrente'}
            ...
            {onglet === 'jour' && (
              <div className="flex items-center gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => e.target.value && setDateSelectionnee(e.target.value)}
                  className="h-9 w-auto font-semibold"
                ...
                {!estAujourdhui && (
                  <Button size="sm" variant="ghost" onClick={() => setDateSelectionnee(isoAujourdhui())}>
                    Aujourd'hui
                  </Button>
                )}
              </div>
          ...
          {onglet === 'jour' ? (
            <>
              ...
              <MotionCard className="space-y-4 p-5">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-sm font-semibold">
                    ...
                    {couverture.total > 0 && couverture.couverts < couverture.total && (
                      <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-destructive/10 px-2.5 py-1 text-xs font-bold text-destructive">
                        <AlertTriangle className="size-3" /> {couverture.total - couverture.couverts} sans agent — avis « sans attribution » possibles
                      </span>
                    )}
                    {couverture.total > 0 && couverture.couverts === couverture.total && (
                      <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1 text-xs font-bold text-primary">
              ...
              {/* Panneau suggestion */}
              {suggestion && (
                <MotionCard className="space-y-3 border-primary/30 p-5">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <p className="flex items-center gap-2 text-sm font-bold">
                      <Sparkles className="size-4 text-primary" />
                      Suggestion
                      {suggestion.source && (
                        <span className="font-normal text-muted-foreground">— base : {suggestion.source}</span>
                      )}
                    </p>
                    <div className="flex gap-2">
                  ...
                  {suggestion.propositions.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Aucune proposition pour ce jour.</p>
                  ) : (
                    <ul className="divide-y divide-border/60 rounded-xl border border-border/60">
                      {suggestion.propositions.map((p: any, i: number) => (
                        <li key={i} className="flex flex-col gap-1 p-3 text-sm sm:flex-row sm:items-center sm:justify-between">
                          <span>
            ...
            <>
              ...
              <MotionCard className="space-y-4 p-5">
                <div className="flex flex-wrap gap-2">
                  {JOURS_ORDRE.map((j) => (
                    <Button key={j} size="sm" variant={jourType === j ? 'default' : 'outline'} onClick={() => setJourType(j)}>
                      {JOURS_LABELS[j]}
                    </Button>
                  ))}
                </div>
                {modelesDuJour.length === 0 ? (
                  <EmptyState
                    icon={LayoutTemplate}
                    title={`Aucun créneau le ${JOURS_LABELS[jourType].toLowerCase()}`}
                    description="Ajoutez ci-dessous les créneaux récurrents : ils serviront à générer le planning en un clic."
                  />
                ) : (
                  <ul className="divide-y divide-border/60 rounded-xl border border-border/60">
                    {modelesDuJour.map((m: any) => (
                      <li key={m.id} className="flex items-center justify-between gap-2 p-3 text-sm">
                        <span>
                          ...
                          {(!m.guichet?.actif || m.guichet?.archive || m.agent?.actif === false) && (
                            <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-destructive/10 px-2 py-0.5 text-xs font-bold text-destructive">
            ...
            <section className="flex flex-col gap-3 rounded-2xl border border-border/70 bg-card/60 p-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="relative w-full sm:max-w-md">
                ...
                <Input
                  ...
                  className="h-10 pl-9 font-semibold"
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          {affectationsGuichet.length === 0 && (
                            <span
                              title="Aucun agent affecté pour ce jour"
