...
export async function backfillScoresReponse(prisma: PrismaClient): Promise<{
  traites: number;
  remplis: number;
...
if (lanceEnCLI) {
  ...
  backfillScoresReponse(prisma)
    .then((r) => {
      console.log(
        `[backfillScoresReponse] traités=${r.traites} remplis=${r.remplis} non-valencés_ignorés=${r.ignoresNonValences}`
      );
    })
    .catch((e) => {
