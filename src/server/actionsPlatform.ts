...
export const creerEntreprise = async (
  ...
  if (existant) {
    const idEntrepriseExistante = (existant as any)?.id_entreprise ?? null;
    throw new HttpError(
      409,
      'Un utilisateur utilise déjà cette adresse email.',
      idEntrepriseExistante ? { entreprise_id: idEntrepriseExistante } : undefined
    );
  }
  ...
  try {
    ...
    try {
      ...
      for (const ident of identites) {
        if ((ident as any)?.auth?.userId === admin.id) {
          await prisma.authIdentity.delete({
            where: {
              providerName_providerUserId: {
                providerName: (ident as any).providerName,
