...
export class NvidiaProvider implements AIProvider {
  ...
  constructor() {
    this.model = process.env.NVIDIA_MODEL || 'qwen/qwen3-next-80b-a3b-instruct';
  ...
  async analyserAvis(commentaire: string, contexte?: ContextAvis): Promise<AnalyseResult> {
    if (!this.client) {
      throw new Error('NVIDIA_API_KEY non configurée dans les variables d’environnement.');
    }
...
Retourne exclusivement le JSON demandé.`;
    ...
    if (!content) {
      throw new Error('Réponse vide du modèle NVIDIA NIM.');
    }
    ...
    try {
      ...
      throw new Error(`JSON malformé retourné par NVIDIA NIM: ${err?.message}`);
    }
