...
export function extraireObjetJson(nom: string, content: string | null | undefined): unknown {
...
export function validerReponseJson<T>(
  nom: string,
  schema: z.ZodType<T>,
  brut: unknown,
): T {
  ...
  if (!parsed.success) {
    ...
    throw new Error(
      `Réponse IA ${nom} non conforme au schéma: ${parsed.error.issues.map((i) => i.path.join('.')).join(', ')} — extrait: ${extrait}`,
    );
  }
  return parsed.data;
}
