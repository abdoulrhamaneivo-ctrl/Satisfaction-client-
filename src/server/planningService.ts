...
export type ResultatGeneration = {
  crees: number;
  ignores: Array<{ guichet: string; agent: string; raison: string }>;
};
...
function jourSemaineUTC(dateStr: string): number {
  return new Date(`${dateStr}T00:00:00.000Z`).getUTCDay(); // 0 dim … 6 sam
}

function ajouterJours(dateStr: string, n: number): string {
...
async function validerLigne(
  ...
  heureDebut: string,
  heureFin: string,
  exclureAffectationId?: number
): Promise<{ nomGuichet: string; nomAgent: string }> {
  ...
  if (!agent || agent.role !== 'AGENT' || agent.actif !== true || agent.id_agence !== idAgence) {
    throw new HttpError(
      400,
      "L'agent n'est plus disponible (désactivé, déplacé ou rôle modifié)."
    );
  }
  ...
  if (conflit) {
    throw new HttpError(
      409,
      `Déjà planifié sur « ${conflit.guichet?.nom_guichet || 'un autre guichet'} » (${conflit.heure_debut}–${conflit.heure_fin}).`
    );
  }
  return {
...
async function creerLignes(
  entities: any,
  idAgence: number,
  dateStr: string,
  lignes: Array<{ id_guichet: number; id_agent: string; heure_debut: string; heure_fin: string }>
): Promise<ResultatGeneration> {
  const resultat: ResultatGeneration = { crees: 0, ignores: [] };
...
export async function genererDepuisModeles(
  entities: any,
  idAgence: number,
  dateDebut: string,
  dateFin: string
): Promise<ResultatGeneration & { jours: number }> {
  const total: ResultatGeneration & { jours: number } = { crees: 0, ignores: [], jours: 0 };
...
export async function reconduireJournee(
  entities: any,
  idAgence: number,
  dateSource: string,
  dateCible: string
...
// ── C. Suggestion auto (heuristique explicable) ───────────────────────
// Source : même jour de semaine dernière si le jour a déjà des
// affectations, sinon la veille. Les agents indisponibles sont remplacés
// par l'agent actif le moins chargé ce jour-là. Rien n'est écrit : le
// chef visualise puis applique (B ou génération).
...
export async function suggererJournee(
  entities: any,
  idAgence: number,
  dateCible: string
): Promise<{ source: string | null; propositions: PropositionPlanning[] }> {
  const memeJourSemaineDerniere = ajouterJours(dateCible, -7);
  const veille = ajouterJours(dateCible, -1);

  const charger = (dateStr: string) =>
    entities.AffectationGuichet.findMany({
  ...
  const dejaPrevus: Array<{ id_agent: string; heure_debut: string; heure_fin: string }> =
    await entities.AffectationGuichet.findMany({
      where: { date_affectation: new Date(dateCible), guichet: { id_agence: idAgence } },
      select: { id_agent: true, heure_debut: true, heure_fin: true },
    });
  const chargeParAgent = new Map<string, number>();
  for (const d of dejaPrevus) chargeParAgent.set(d.id_agent, (chargeParAgent.get(d.id_agent) || 0) + 1);

  const agentsActifs: Array<{ id: string; nom: string | null; prenom: string | null }> =
    await entities.User.findMany({
      where: { id_agence: idAgence, role: 'AGENT', actif: true },
      select: { id: true, nom: true, prenom: true },
    });

  const propositions: PropositionPlanning[] = [];
  for (const l of lignes) {
    const agentOk =
      l.agent && l.agent.role === 'AGENT' && l.agent.actif === true && l.agent.id_agence === idAgence;
    const guichetOk = l.guichet && l.guichet.actif === true && l.guichet.archive !== true;
    if (!guichetOk) continue; // guichet fermé : on ne propose rien dessus

    if (agentOk) {
      // Conflit avec ce qui est déjà prévu ce jour-là ? → on signale.
      const conflit = dejaPrevus.some(
        (d: any) =>
          d.id_agent === l.id_agent && chevauche(d.heure_debut, d.heure_fin, l.heure_debut, l.heure_fin)
      );
      propositions.push({
        id_guichet: l.id_guichet,
    ...
    const candidats: Array<{ id: string; nom: string | null; prenom: string | null; charge: number }> =
      agentsActifs
        .map((a) => ({
          id: a.id,
          nom: a.nom,
...
export async function appliquerPropositions(
  entities: any,
  idAgence: number,
  dateCible: string,
  lignes: Array<{ id_guichet: number; id_agent: string; heure_debut: string; heure_fin: string }>
