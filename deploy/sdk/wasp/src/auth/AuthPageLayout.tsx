  ...
  return (
    <AmbientBackground className="flex items-center justify-center px-4 py-10 sm:py-12">
      ...
      <motion.div
        ...
        className="mb-5 w-full max-w-4xl rounded-2xl bg-card px-4 py-3 shadow-sm ring-1 ring-border/60 lg:hidden"
      >
      ...
      >
        ...
        <div className="relative hidden flex-col justify-between overflow-hidden bg-brand-green-deeper p-10 text-white lg:flex">
          ...
          <motion.div
            aria-hidden
            className="absolute -right-16 top-10 h-64 w-64 rounded-full bg-primary/25 blur-3xl"
            animate={prefersReducedMotion ? undefined : { x: [0, 40, -20, 0], y: [0, -30, 20, 0], scale: [1, 1.15, 1, 1] }}
            transition={{ duration: 18, repeat: Infinity, ease: 'easeInOut' }}
          />
          <motion.div
            aria-hidden
            className="absolute -left-10 bottom-16 h-56 w-56 rounded-full bg-warning/20 blur-3xl"
            animate={prefersReducedMotion ? undefined : { x: [0, -30, 25, 0], y: [0, 25, -20, 0], scale: [1, 1.1, 0.95, 1] }}
            transition={{ duration: 22, repeat: Infinity, ease: 'easeInOut', delay: 3 }}
          />
          <motion.div
            aria-hidden
            className="absolute right-8 bottom-8 h-40 w-40 rounded-full bg-white/10 blur-3xl"
            animate={prefersReducedMotion ? undefined : { x: [0, 20, -15, 0], y: [0, -20, 15, 0] }}
        ...
        <div className="relative bg-card p-8 sm:p-10">
          <div className="mb-6 flex justify-start">
            <a
              href="/"
              className="inline-flex items-center gap-1.5 rounded-lg px-1 py-0.5 text-sm font-semibold text-muted-foreground transition-colors hover:text-foreground group cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
            >
          ...
          <motion.div
            initial={prefersReducedMotion ? false : { opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: prefersReducedMotion ? 0 : 0.1, duration: prefersReducedMotion ? 0 : 0.35 }}
            className="mb-8"
          >
          ...
          <motion.div
            initial={prefersReducedMotion ? false : { opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: prefersReducedMotion ? 0 : 0.18, duration: prefersReducedMotion ? 0 : 0.35 }}
          >
            {children}
          </motion.div>

          {footer && (
            <div className="mt-6 border-t border-border/60 pt-5 text-sm font-medium text-muted-foreground">{footer}</div>
          )}
        </div>
      </motion.div>
    </AmbientBackground>
