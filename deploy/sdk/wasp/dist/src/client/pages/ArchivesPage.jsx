...
import { getArchives, desarchiverGuichet, desarchiverAgence, desarchiverAlerte, desarchiverTache, } from 'wasp/client/operations';
import { motion, AnimatePresence } from 'framer-motion';
import { Archive, Building2, Store, AlertTriangle, CheckCircle2, RotateCcw, MapPin, Search, } from 'lucide-react';
 ...
 */
export const ArchivesPage = () => (<RequireAuth>
...
function ArchivesContent() {
    ...
    const [onglet, setOnglet] = useState('guichets');
    ...
    const restaurer = async (cle, fn, libelle) => {
        if (busyId)
            return;
        setBusyId(cle);
        try {
      ...
      <div className="sticky top-16 lg:top-4 z-20 mb-6 rounded-2xl border border-border/70 bg-card/95 p-4 shadow-sm ">
        <div className="flex flex-wrap gap-2 border-b border-border/70 pb-3">
          {ONGLETS.map((o) => (<Button key={o.key} type="button" variant={onglet === o.key ? 'default' : 'ghost'} onClick={() => setOnglet(o.key)} className={onglet === o.key ? 'rounded-full shadow-premium' : 'rounded-full text-muted-foreground'}>
              {o.icon}
              {o.label}
              <span className={`rounded-full px-1.5 py-0.5 text-[11px] font-bold ${onglet === o.key ? 'bg-white/20' : 'bg-muted-foreground/10'}`}>
                {compteurs[o.key]}
              </span>
            </Button>))}
        </div>
        ...
        <div className="relative mt-4 max-w-md">
          ...
          <Input value={recherche} onChange={(event) => setRecherche(event.target.value)} placeholder={`Rechercher dans les ${onglet} archivés…`} className="h-10 pl-9" aria-label="Rechercher dans les archives"/>
        </div>
      </div>

      {isLoading ? (<div className="space-y-3">
          {[0, 1, 2].map((i) => (<div key={i} className="h-16 animate-pulse rounded-xl border border-border/60 bg-card-subtle/50"/>))}
        </div>) : (<AnimatePresence mode="wait">
          {onglet === 'guichets' && (<ListeArchive key="guichets" items={guichets} recherche={recherche} vide={{ icon: Store, title: 'Aucun guichet archivé', description: "Les guichets fermés définitivement apparaîtront ici." }} render={(g) => ({
                    titre: g.nom_guichet,
          ...
          {onglet === 'agences' && (<ListeArchive key="agences" items={agences} recherche={recherche} vide={{ icon: Building2, title: 'Aucune agence archivée', description: "Les agences fermées définitivement apparaîtront ici." }} render={(a) => ({
                    titre: a.nom_agence,
                    sousTitre: a.commune,
          ...
          {onglet === 'alertes' && (<ListeArchive key="alertes" items={alertes} recherche={recherche} vide={{ icon: AlertTriangle, title: 'Aucune alerte archivée', description: "Les alertes traitées depuis plus de 6 mois y sont déplacées automatiquement." }} render={(al) => ({
                    titre: al.message,
          ...
          {onglet === 'taches' && (<ListeArchive key="taches" items={taches} recherche={recherche} vide={{ icon: CheckCircle2, title: 'Aucune tâche archivée', description: "Les tâches terminées depuis plus de 6 mois y sont déplacées automatiquement." }} render={(t) => ({
                    titre: t.titre,
...
function ListeArchive({ items, recherche, vide, render, }) {
    ...
    return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-2.5">
      {itemsFiltres.map((item, i) => {
            const { titre, sousTitre, date, busy, onRestaurer } = render(item);
            return (<MotionCard key={item.id?.toString?.() ?? i} interactive={false} className="flex flex-wrap items-center justify-between gap-3 p-4">
            <div className="min-w-0">
              <p className="truncate font-semibold text-foreground">{titre}</p>
              <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-muted-foreground">
                {sousTitre && (<span className="flex items-center gap-1">
                    <MapPin className="size-3"/> {sousTitre}
                  </span>)}
                <span>Archivé le {formatDate(date)}</span>
              </div>
            </div>
            <Button type="button" variant="outline" size="sm" onClick={onRestaurer} disabled={busy} className="shrink-0">
...
//# sourceMappingURL=ArchivesPage.jsx.map