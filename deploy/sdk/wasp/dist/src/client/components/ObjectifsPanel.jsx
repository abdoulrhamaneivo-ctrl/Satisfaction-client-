...
export const ObjectifsPanel = ({ selectedAgenceId }) => {
    ...
    const getObjectifForCritere = (idCritere) => objectifsList.find((o) => o.id_critere === idCritere);
    const getEdit = (idCritere) => {
        if (editMap[idCritere])
            return editMap[idCritere];
    ...
    const handleSave = async (idCritere) => {
    ...
    const handleDelete = async (idCritere) => {
    ...
    if (activeCriteres.length === 0) {
        return (<MotionCard className="p-6">
    ...
    return (<MotionCard className="p-6 space-y-5">
            ...
            return (<motion.div key={critere.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="rounded-xl border border-border/70 bg-background p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                {objectifActuel && (() => {
                    ...
                    const tone = statut === 'Actif'
                        ? 'bg-success/10 text-success'
                        : statut === 'Expiré'
                            ? 'bg-muted text-muted-foreground'
                            : 'bg-warning/10 text-warning';
                    return (<span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${tone}`}>
                      {statut}
                    </span>);
                })()}
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="h-2 overflow-hidden rounded-full bg-muted">
                  <motion.div className="h-full rounded-full bg-primary" initial={{ width: 0 }} animate={{ width: `${Math.min(100, valeurNum)}%` }} transition={{ duration: 0.5, ease: 'easeOut' }}/>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <Input type="number" min={0} max={100} value={edit.valeur} onChange={(e) => setEditMap((prev) => ({ ...prev, [critere.id]: { ...getEdit(critere.id), valeur: e.target.value } }))} className="h-9"/>
                </div>
                <div>
                  <label className="block text-[10px] font-semibold uppercase text-muted-foreground mb-0.5">Début</label>
                  <Input type="date" value={edit.debut} onChange={(e) => setEditMap((prev) => ({ ...prev, [critere.id]: { ...getEdit(critere.id), debut: e.target.value } }))} className="h-9 text-xs"/>
                </div>
                <div>
                  <label className="block text-[10px] font-semibold uppercase text-muted-foreground mb-0.5">Fin</label>
                  <Input type="date" value={edit.fin} onChange={(e) => setEditMap((prev) => ({ ...prev, [critere.id]: { ...getEdit(critere.id), fin: e.target.value } }))} className="h-9 text-xs"/>
                </div>
              </div>

              <motion.div whileTap={{ scale: 0.97 }} className="flex gap-2">
                {objectifActuel && (<Button size="sm" variant="outline" className="flex-none border-destructive/40 text-destructive hover:bg-destructive/10" disabled={deleting === critere.id} onClick={() => setObjectifAConfirmer({ idCritere: critere.id, libelle: critere.libelle_critere })} title="Supprimer l'objectif" aria-label="Supprimer l'objectif">
                    <Trash2 className="size-3.5"/>
                  </Button>)}
                <Button size="sm" className="flex-1" disabled={saving === critere.id} onClick={() => handleSave(critere.id)}>
      ...
      <AlertDialog open={objectifAConfirmer !== null} onOpenChange={(open) => !open && setObjectifAConfirmer(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction variant="destructive" onClick={() => objectifAConfirmer && handleDelete(objectifAConfirmer.idCritere)}>
              Supprimer
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
...
//# sourceMappingURL=ObjectifsPanel.jsx.map