...
// ============================================================================
export function normaliserNumeroCI(numeroBrut) {
    const nettoye = numeroBrut.trim().replace(/[\s.\-()]/g, '');
    if (nettoye.startsWith('+'))
        return nettoye; // déjà au format international
    if (nettoye.startsWith('00'))
        return '+' + nettoye.slice(2); // 00225... → +225...
    if (nettoye.startsWith('225') && nettoye.length === 13)
        return '+' + nettoye; // 225... sans le +
    if (/^0\d{9}$/.test(nettoye))
        return '+225' + nettoye; // format local à 10 chiffres
 ...
 */
export async function envoyerAlerteSMS(destinataire, message) {
 ...
 */
export async function envoyerAlerteWhatsApp(destinataire, message) {
    const numero = normaliserNumeroCI(destinataire);
