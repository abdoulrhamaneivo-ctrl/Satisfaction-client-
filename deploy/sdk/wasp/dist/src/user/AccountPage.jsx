...
function ProfilSection({ user }) {
    ...
    return (<Card>
      ...
      <CardContent className="space-y-5">
        ...
        {emailModifie && (<div className="space-y-1.5 rounded-lg border border-border/70 bg-card-subtle/40 p-3">
            <Label htmlFor="password-email">Confirmez avec votre mot de passe</Label>
            <PasswordInput id="password-email" value={passwordPourEmail} onChange={(e) => setPasswordPourEmail(e.target.value)} placeholder="Mot de passe actuel"/>
          </div>)}

        {emailModifie && (<div className="flex justify-end">
...
function SecuriteSection({ user: _user }) {
    const { toast } = useToast();
