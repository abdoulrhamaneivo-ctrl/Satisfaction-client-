    ...
    return (<DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenuContent align="end" className="w-56 rounded-xl border border-border/80 shadow-premium p-1 space-y-0.5">
        ...
        <DropdownMenuItem asChild className="rounded-lg cursor-pointer">
          <WaspRouterLink to="/settings" onClick={() => setOpen(false)} className="flex w-full items-center gap-2.5 px-2.5 py-2 text-xs font-semibold text-foreground hover:text-primary">
            <Settings className="size-4 text-primary"/>
            Paramètres & Clés IA
          </WaspRouterLink>
        </DropdownMenuItem>
        <DropdownMenuItem asChild className="rounded-lg cursor-pointer text-destructive hover:text-destructive hover:bg-destructive/10">
