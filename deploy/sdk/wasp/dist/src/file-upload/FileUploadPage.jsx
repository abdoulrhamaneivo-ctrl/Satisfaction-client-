...
import { addFileToDb, createFileUploadUrl, deleteFile, getAllFilesByUser, getDownloadFileSignedURL, useQuery, } from "wasp/client/operations";
...
export function FileUploadPage() {
    ...
    const { isLoading: isDownloadUrlLoading, refetch: refetchDownloadUrl } = useQuery(getDownloadFileSignedURL, { s3Key: fileKeyForS3 }, { enabled: false });
    useEffect(() => {
    ...
    return (<>
      <div className="py-10 lg:mt-10">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <Card className="my-8">
            <CardContent className="mx-auto my-10 space-y-10 px-4 py-8 sm:max-w-lg">
              <form onSubmit={handleUpload} className="flex flex-col gap-4">
                <div className="space-y-2">
                  <Label htmlFor="file-upload" className="text-foreground text-sm font-medium">
                    Select a file to upload
                  </Label>
                  <Input type="file" id="file-upload" name="file-upload" accept={ALLOWED_FILE_TYPES.join(",")} className="cursor-pointer"/>
                </div>
                <div className="space-y-2">
            ...
            !allUserFiles.isLoading ? (<div className="space-y-3">
                    {allUserFiles.data.map((file) => (<Card key={file.s3Key} className="p-4">
                        <div className={cn("flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center", {
                    "opacity-70": file.s3Key === fileKeyForS3 &&
                        isDownloadUrlLoading,
                          ...
                          <div className="flex items-center justify-end gap-2">
                            <Button onClick={() => setFileKeyForS3(file.s3Key)} disabled={file.s3Key === fileKeyForS3 &&
                    isDownloadUrlLoading} variant="outline" size="sm">
                              <Download className="h-5 w-5"/>
                            </Button>
                            <Button onClick={() => setFileToDelete(file)} variant="outline" size="sm" aria-label="Delete file">
      ...
      {fileToDelete && (<Dialog open={!!fileToDelete} onOpenChange={(isOpen) => !isOpen && setFileToDelete(null)}>
          <DialogContent>
            <DialogHeader>
            ...
            <DialogFooter>
              ...
              <Button variant="destructive" onClick={() => handleDelete(fileToDelete)}>
                Delete
              </Button>
            </DialogFooter>
          </DialogContent>
