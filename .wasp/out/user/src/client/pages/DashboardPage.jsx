...
export const DashboardPage = () => {
    ...
    const { data: experience, isLoading: loadingExperience } = useQuery(getIndicateursExperience, { nbJours: periodeJours }, { enabled: aUnTenant });
    ...
    const npsAgregat = experience?.agregats?.nps && typeof experience.agregats.nps.nps === 'number'
        ? {
            nps: experience.agregats.nps.nps,
            volume: experience.agregats.nps.volume ?? 0,
        }
        : null;
    ...
    return (<RequireEnterpriseRole>
      ...
      <AmbientBackground>
        <PageShell>
          ...
          {experience?.agregats && (<section aria-label="Expérience client">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5 lg:gap-5">
                ...
                <div title="Volumes : avis = soumissions distinctes ; commentaires = lignes avec texte.">
                  <StatCard title="Volume" value={loadingExperience ? '…' : `${experience.agregats.volumeAvis} avis`} icon={MessageSquare} accent="primary" index={3} trend={experience.agregats.evolutionVolumePct != null ? formatDelta(experience.agregats.evolutionVolumePct, '%') : undefined} trendDirection={(experience.agregats.evolutionVolumePct ?? 0) >= 0 ? 'up' : 'down'}/>
                </div>
                ...
                (experience.agregats.qualiteDonneesDetails
                    ...
                    : '')}>
                  <StatCard title="Confiance" value={loadingExperience ? '…' : experience.agregats.confiance === 'ELEVEE' ? 'Élevée' : experience.agregats.confiance === 'MOYENNE' ? 'Moyenne' : 'Faible'} icon={CheckCircle2} accent={experience.agregats.confiance === 'ELEVEE' ? 'success' : 'secondary'} index={4}/>
                </div>
              </div>
              ...
              {experience.agregats.ces && experience.agregats.ces.volume > 0 && (<div className="mt-4 grid gap-4 sm:grid-cols-3">
                  <StatCard title="Effort perçu (top box)" value={loadingExperience ? '…' : `${Math.round(experience.agregats.ces.top_box)} %`} icon={Gauge} accent="success" index={0}/>
                  <StatCard title={`CES moyen (1 = très facile, max ${experience.agregats.ces.echelle})`} value={loadingExperience ? '…' : experience.agregats.ces.note_effort_moyenne != null ? `${experience.agregats.ces.note_effort_moyenne}/${experience.agregats.ces.echelle}` : 'N/A'} icon={Scale} accent="secondary" index={1}/>
                  <StatCard title="Effort élevé" value={loadingExperience ? '…' : `${Math.round(experience.agregats.ces.taux_effort_eleve)} %`} icon={AlertTriangle} accent={experience.agregats.ces.taux_effort_eleve > 25 ? 'destructive' : 'primary'} index={2}/>
                </div>)}
          ...
          {!isLoading && !estDirectionPure && (<section>
                  ...
                  {avisGroupes.slice(0, 5).map((avis) => {
                    ...
                    aria-label={`Ouvrir l’avis du ${premiere.guichet?.nom_guichet || 'guichet inconnu'}, note ${avis.score_moyen ?? 'non chiffrée'}/5`}>
                        <td className="px-6 py-4">
                          <span className={`rounded-full px-2.5 py-1 text-xs font-bold border ${avis.score_moyen == null
              ...
              <div className="space-y-2">
                ...
                return (<div key={a.id_agence} className="rounded-xl border border-border/60 bg-card/70 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          <p className="truncate text-sm font-bold font-satoshi text-foreground">
                            {a.nom_agence}
                            {comparaisonAgences.meilleure_agence === a.nom_agence && a.nb_avis > 0 && (<span className="ml-2 rounded-full bg-success/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-success-strong">Meilleure</span>)}
                            {comparaisonAgences.agence_a_surveiller === a.nom_agence && a.nb_avis > 0 && comparaisonAgences.agences.filter((x) => x.nb_avis > 0).length > 1 && (<span className="ml-2 rounded-full bg-warning/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-warning-strong">À surveiller</span>)}
                          </p>
                        ...
                        <div className="shrink-0 text-right">
                          ...
                          <p className="text-[11px] font-semibold text-muted-foreground">
                            ...
                            {delta !== null && delta !== undefined && (<span className={`ml-1.5 font-bold ${delta >= 0 ? 'text-success-strong' : 'text-destructive-strong'}`}>
                      ...
                      {a.score_moyen !== null && (<div className="mt-2 h-1.5 overflow-hidden rounded-full bg-muted/60">
                          <div className={`h-full rounded-full ${a.score_moyen >= 4 ? 'bg-success' : a.score_moyen >= 3 ? 'bg-warning' : 'bg-destructive'}`} style={{ width: `${(a.score_moyen / 5) * 100}%` }}/>
                        </div>)}
                    </div>);
            })}
              </div>
