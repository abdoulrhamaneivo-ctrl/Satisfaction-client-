...
export function useQuery(queryFn, args, options) {
    if (typeof queryFn !== 'function') {
        ...
        throw new Error(`useQuery a recu une operation qui n'est pas une fonction (recu: ${typeof queryFn}, ` +
...
export function useAction(actionFn, _options) {
    const mutation = useMutation(actionFn);
    return async (args) => mutation.mutateAsync(args);
}
export const getFormDefinitionForGuichet = vi.fn(async (_args) => null);
export const soumettreAvis = vi.fn(async (_args) => ({ id: '1' }));
export const completerSoumission = vi.fn(async (_args) => ({ ok: true }));
