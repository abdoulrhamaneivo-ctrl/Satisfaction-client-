...
export function NavBar({ navigationItems, }) {
    ...
    const filterByRole = (item) => {
        if (item.children) {
            const visibleChildren = item.children.filter((child) => !child.roles || (currentUser && child.roles.includes(currentUser.role)));
    ...
    return (<>
      <header className={cn("fixed top-0 left-0 right-0 z-50 w-full border-b border-border/80 bg-card/95 shadow-sm transition-all duration-200", isScrolled && "shadow-md bg-card/95")}>
        <div className="mx-auto max-w-[1440px]">
          <nav className="flex h-[68px] items-center justify-between px-4 lg:px-8" aria-label="Global">
            <div className="flex items-center gap-8">
              <WaspRouterLink to={currentUser ? routes.DashboardRoute.to : routes.LoginRoute.to} className="flex items-center text-foreground transition-colors hover:text-primary">
            ...
            <NavBarMobileMenu isScrolled={isScrolled} navigationItems={visibleNavigationItems} currentPath={location.pathname}/>
...
function NavBarDesktopUserDropdown({ isScrolled }) {
    ...
    return (<div className="hidden items-center justify-end gap-3 lg:flex lg:flex-1">
      <ul className="flex items-center justify-center gap-2 sm:gap-4">
        ...
        {!!user && total > 0 && (<li>
            <ReactRouterLink to="/alertes-taches" title={`${total} action${total > 1 ? 's' : ''} en attente`} className="relative flex items-center justify-center p-2 rounded-xl border border-border/60 bg-muted/30 hover:bg-muted transition-colors">
              <Bell className={cn('transition-colors', isScrolled ? 'size-4' : 'size-4.5', hasCritical ? 'text-destructive animate-pulse' : 'text-warning')}/>
...
function NavBarMobileMenu({ isScrolled, navigationItems, currentPath, }) {
    const { data: user, isLoading: isUserLoading } = useAuth();
    ...
    return (<div className="flex lg:hidden">
      <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
        <SheetTrigger asChild>
          <Button type="button" variant="ghost" size="icon" className="text-muted-foreground hover:text-muted">
        ...
        <SheetContent side="right" className="flex w-[300px] flex-col p-0 sm:w-[400px]">
...
function renderNavigationItems(navigationItems, setMobileMenuOpen, currentPath) {
    const menuStyles = cn({
        "block rounded-xl px-3 py-2 text-sm font-medium leading-7 text-foreground hover:bg-muted hover:text-primary transition-colors": !!setMobileMenuOpen,
        "rounded-xl px-3.5 py-2 text-sm font-bold text-muted-foreground transition-all duration-200 hover:bg-muted/70 hover:text-foreground": !setMobileMenuOpen,
    });
    return navigationItems.map((item) => {
        ...
        if (item.children && item.children.length > 0) {
                ...
                <AccordionContent className="pl-3">
                  <ul className="space-y-1 border-l border-border/60 pl-3">
                    {item.children.map((child) => (<li key={child.name}>
                        <ReactRouterLink to={child.to} className={cn(menuStyles, currentPath === child.to && 'bg-primary/10 text-primary font-bold')} aria-current={currentPath === child.to ? 'page' : undefined} onClick={() => setMobileMenuOpen(false)}>
                          {child.name}
                        </ReactRouterLink>
                      </li>))}
                  </ul>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="rounded-xl border border-border/80 shadow-premium p-1">
              {item.children.map((child) => (<DropdownMenuItem key={child.name} asChild className="rounded-lg">
                  <ReactRouterLink to={child.to} className={cn(currentPath === child.to && 'bg-primary/10 text-primary font-bold')} aria-current={currentPath === child.to ? 'page' : undefined}>
                    {child.name}
                  </ReactRouterLink>
                </DropdownMenuItem>))}
            </DropdownMenuContent>
        ...
        return (<li key={item.name}>
        <ReactRouterLink to={item.to} className={cn(menuStyles, activeStyles)} aria-current={isActive ? 'page' : undefined} onClick={setMobileMenuOpen ? () => setMobileMenuOpen(false) : undefined} target={item.to.startsWith("http") ? "_blank" : undefined}>
          {item.name}
        </ReactRouterLink>
      </li>);
    });
}
function NavLogo({ isScrolled: _isScrolled }) {
    return (<BrandLogo className="size-8 transition-transform duration-300 hover:scale-105"/>);
}
