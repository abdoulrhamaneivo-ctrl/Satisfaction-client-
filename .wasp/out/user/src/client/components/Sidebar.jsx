...
import { Inbox, Calendar, Store, Users, Settings, LayoutDashboard, SlidersHorizontal, Building2, Archive, MessageSquareQuote, Search, LogOut, ShieldCheck, PlusCircle, ScrollText, Lock, UserRound, Sparkles, } from 'lucide-react';
...
function navLinkClass(isActive) {
    return cn('relative w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all duration-200 cursor-pointer', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-card', isActive
...
export function SidebarContent({ onNavigate, className }) {
    ...
    return (<div className={cn('flex h-full flex-col justify-between overflow-y-auto momentum-scroll select-none text-foreground', className)}>
      <div className="p-4 space-y-5">
        <Link to="/dashboard" onClick={onNavigate} className="flex items-center justify-between p-2.5 rounded-2xl bg-muted/40 border border-border/70 hover:bg-muted/70 transition-all cursor-pointer group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <div className="flex items-center gap-3">
        ...
        <div className="pt-2 space-y-1">
          ...
          {(!isDirection || user?.id_agence != null) && (<NavItem to="/avis" icon={MessageSquareQuote} label="Avis & CSAT" isActive={isCurrent('/avis')} onNavigate={onNavigate}/>)}
