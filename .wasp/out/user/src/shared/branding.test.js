...
import { BRANDING, fondWhiteLabelRecevable, varianteTextePourFond, ratioContrasteHsl, foregroundPourAplat, } from './branding';
/** Convertit un token « H S% L% » en triplet RVB normalisé. */
const hslVersRgb = (token) => {
...
const luminance = (token) => {
    const [r, v, b] = hslVersRgb(token).map((canal) => canal <= 0.03928 ? canal / 12.92 : ((canal + 0.055) / 1.055) ** 2.4);
 ...
 */
const composerSur = (accent, alpha, fond) => {
...
describe('Charte — contrastes WCAG 2.2 AA (Vague 4)', () => {
    ...
    test('le vert de marque en TEXTE sur crème reste sous le seuil — d’où les variantes -strong', () => {
        ...
        expect(ratio(BRANDING.color_primary_strong, FOND_PAGE)).toBeGreaterThanOrEqual(4.5);
    });
    ...
    test('le texte d\'une option sélectionnée atteint 4,5:1 sur teinte /10, /15 et /25', () => {
        for (const [teinte, texte, libelle] of TEINTES) {
            for (const alpha of [0.1, 0.15, 0.25]) {
                ...
                expect(r, `${libelle} : texte sur ${teinte}/${Math.round(alpha * 100)} → ${r.toFixed(2)}:1`).toBeGreaterThanOrEqual(4.5);
            }
        }
    });
