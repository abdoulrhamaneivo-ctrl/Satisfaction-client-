...
export async function serveStaticClient({ app }) {
    ...
    console.log(`[static] trust proxy = ${proxys} (${typeof proxys === 'number' ? 'n proxies de confiance' : 'confiance totale'})`);
    ...
    try {
        ...
        if (m)
            hashR = 'sha256-' + crypto.createHash('sha256').update(m[1], 'utf8').digest('base64');
    }
    catch {
    ...
    const déplacerCouchesAvantRouteurWasp = () => {
        ...
        if (!Array.isArray(stack) || longueurAvantInstallation === null)
            return;
