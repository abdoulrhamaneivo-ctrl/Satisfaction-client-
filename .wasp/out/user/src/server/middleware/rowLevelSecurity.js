...
export function requireRole(context, roles) {
    ...
    if (userRole === 'QUALITE') {
        throw new HttpError(403, "Votre rôle 'QUALITE' n'existe plus (fusionné dans Chef d'agence). Demandez à la Direction de recréer votre compte.");
    }
...
export function estDirectionCumulee(user) {
    return !!user && user.role === 'DIRECTION' && user.id_agence != null;
}
export function estDirectionPure(user) {
    return !!user && user.role === 'DIRECTION' && user.id_agence == null;
}
/**
 ...
 */
export function voitVerbatim(user) {
    if (!user?.role)
        return false;
    if (user.role === 'DIRECTION')
        return user.id_agence != null;
