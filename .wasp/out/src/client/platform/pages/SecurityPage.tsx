...
function SecurityInner() {
  ...
  useEffect(() => {
    if (me?.totp_actif) setMfaActive(true)
  }, [me?.totp_actif])
  ...
  async function activerMfa() {
    setMfaErreur(null); setMfaMessage(null)
    try {
  ...
  async function soumettre() {
    ...
    try {
      ...
      setEmail(''); setPrenom(''); setNom(''); setTotpCode('')
    } catch (e: any) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <section className="rounded-2xl border border-warning/40 bg-warning/5 p-6">
        ...
        {totpActif ? (
          <p className="mt-2 text-sm font-semibold text-success">La 2FA est activée pour votre compte.</p>
        ) : (
          <>
      ...
      <section className="rounded-2xl border border-border/80 bg-card p-6">
        ...
        {message && (
          <div className="mt-4 flex items-center gap-2 rounded-xl border border-success/30 bg-success/10 p-3 text-sm font-bold text-success">
            <CheckCircle2 className="size-4" /> {message}
          </div>
        )}
        {erreur && (
          <div className="mt-4 flex items-center gap-2 rounded-xl border border-destructive/30 bg-destructive/10 p-3 text-sm font-bold text-destructive">
        ...
        <button
          onClick={soumettre}
          disabled={!emailValide || !prenom.trim() || !nom.trim() || !codeValide || !totpActif || envoi}
          className="mt-4 inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-primary-foreground hover:bg-primary/90 disabled:opacity-50"
        >
      ...
      <section className="rounded-2xl border border-border/80 bg-card p-6">
        ...
        <ul className="mt-4 space-y-3">
          {mesures.map(({ icon: Icon, label, ok }) => (
