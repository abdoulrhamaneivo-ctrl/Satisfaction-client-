...
export const BrandProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const { data: brandingServeur } = useQuery(
    getBranding,
    undefined,
    { enabled: !!user?.id_entreprise } as any,
  );

  const brandConfig: BrandConfigType = useMemo(() => {
    const s: any = brandingServeur;
    if (!s) return BRANDING;
    ...
    const texte = (v: any, defaut: string) =>
  ...
  return (
    <BrandContext.Provider value={{ brandConfig, isLoading: false }}>
...
function AppliqueThemeMarque({ brandConfig, children }: { brandConfig: BrandConfigType; children?: React.ReactNode }) {
  useEffect(() => {
