...
export const EditeurOptions: React.FC<EditeurOptionsProps> = ({ options, onChange, pondere }) => {
  const set = (cle: string, patch: Partial<OptionForm>) => {
  ...
  const retirer = (cle: string) => {
    if (options.length <= 2) return; // minimum serveur : 2 choix
  ...
  return (
    <div className="space-y-2">
      ...
      {options.map((o, i) => (
        <div key={o.cle} className="flex items-center gap-2">
          <span className="w-6 shrink-0 text-center text-xs font-black text-muted-foreground">{i + 1}</span>
          <Input
            value={o.libelle}
            onChange={(e) => set(o.cle, { libelle: e.target.value })}
            placeholder={`Choix ${i + 1}`}
            aria-label={`Libellé du choix ${i + 1}`}
            className="h-10 flex-1 text-sm"
          />
          <Select
            value={o.score === null ? 'auto' : String(o.score)}
            onValueChange={(v) => set(o.cle, { score: v === 'auto' ? null : Number(v) })}
          >
          ...
          {pondere && (
            <Input
              type="number"
              value={o.poids ?? ''}
              onChange={(e) => set(o.cle, { poids: e.target.value === '' ? null : Number(e.target.value) })}
              placeholder="±"
              title="Poids (−100 à +100) : ex. −25. Base 100 + somme, borné 0-100."
              aria-label={`Poids du choix ${i + 1}`}
              className="h-10 w-[72px] shrink-0 text-sm"
            />
          )}
          <Input
            value={o.code_metier}
            onChange={(e) => set(o.cle, { code_metier: e.target.value })}
            placeholder="Code"
            title="Code métier (ex. EXCLUSIF pour « Aucun problème », incompatible avec tout autre choix)."
            aria-label={`Code métier du choix ${i + 1}`}
            className="h-10 w-[92px] shrink-0 text-xs uppercase"
          />
          <Button
            type="button"
            variant="ghost"
            size="icon"
            onClick={() => retirer(o.cle)}
            disabled={options.length <= 2}
            aria-label={`Retirer le choix ${i + 1}`}
            className="min-h-10 min-w-10 shrink-0 hover:bg-destructive/10 hover:text-destructive"
          >
