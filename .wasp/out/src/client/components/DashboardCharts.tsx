...
function ChartSkeletonBars({ variant }: { variant: ChartSkeletonVariant }) {
  if (variant === 'radar') {
    return (
  ...
  if (variant === 'area') {
    return (
      <div className="relative flex flex-1 flex-col justify-end gap-2 pt-6">
        ...
        <div className="flex h-full items-end justify-between gap-2 px-2">
          {[42, 58, 48, 72, 55, 68, 50, 62].map((h, i) => (
            <Skeleton
              key={i}
              className="w-full rounded-t-md bg-gradient-to-t from-brand-green-deep/25 to-primary/15"
              style={{ height: `${h}%` }}
            />
          ))}
        </div>
  ...
  if (variant === 'heatmap') {
    return (
      <div className="grid flex-1 grid-cols-7 gap-1.5 pt-2">
        {Array.from({ length: 42 }).map((_, i) => (
          <Skeleton
            key={i}
            className={cn(
              'aspect-square rounded-sm',
              i % 5 === 0 ? 'bg-primary/20' : 'bg-brand-green-deep/15',
            )}
          />
        ))}
      </div>
  ...
  return (
    <div className="flex flex-1 items-end justify-between gap-3 px-2 pb-2">
      {[55, 78, 42, 90, 65].map((h, i) => (
        <Skeleton
          key={i}
          className="w-full rounded-t-lg bg-gradient-to-t from-brand-green-deep/25 to-primary/15"
          style={{ height: `${h}%` }}
        />
      ))}
    </div>
...
export function ChartSkeleton({
  ...
  className,
  heightClass = 'h-72',
  label = 'Chargement du graphique en cours',
}: ChartSkeletonProps) {
  return (
    <div
      role="status"
      ...
      aria-label={label}
      className={cn(
        'flex flex-col rounded-2xl border border-border/70 bg-card p-5 shadow-premium',
        heightClass,
        className,
      )}
    >
      ...
      {subtitle !== undefined ? (
        <Skeleton className="mb-4 h-3 w-56 rounded-md opacity-80" />
      ) : (
        <div className="mb-4" />
      )}
...
export const HistogrammeSatisfactionSkeleton = () => (
  <ChartSkeleton
    variant="bar"
    subtitle=""
    label="Chargement de la répartition des notes"
  />
);

export const RadarQualiteSkeleton = () => (
  <ChartSkeleton
    variant="radar"
    label="Chargement du radar de maturité"
  />
);

export const TendanceMensuelleSkeleton = () => (
  <ChartSkeleton
    variant="area"
    label="Chargement de la tendance mensuelle"
  />
);

export const ClassementGuichetsSkeleton = () => (
  <ChartSkeleton
    variant="horizontalBar"
    heightClass="min-h-72"
    label="Chargement du classement des guichets"
  />
);

export const ComparaisonAgentsSkeleton = () => (
  <ChartSkeleton
    variant="horizontalBar"
    heightClass="h-64"
    label="Chargement de la comparaison des agents"
  />
);

export const HeatmapReponsesSkeleton = () => (
  <ChartSkeleton
    variant="heatmap"
    heightClass="h-96"
    label="Chargement de la heatmap d'affluence"
  />
);
...
export const HistogrammeSatisfaction = ({ data }: { data: any[] }) => {
  ...
  return (
    <div className="h-72 rounded-2xl border border-border/70 bg-card p-5 shadow-premium">
      ...
      <TableauAccessible
        legende="Répartition des notes sur 5"
        entetes={['Note', 'Nombre de réponses']}
        lignes={counts.map((c) => [c.name, c.count])}
      />
    </div>
  );
};
    ...
    <div className="h-72 rounded-2xl border border-border/70 bg-card p-5 shadow-premium">
      ...
      <div
        ...
        aria-label={`Maturité du pilotage, notée de 0 à 100 : ${data
          .map((d: any) => `${d.subject} : ${d.A ?? d.valeur ?? 0}`)
          .join(', ')}.`}
      >
        <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
      ...
      <TableauAccessible
        legende="Maturité du pilotage, sur 100"
        entetes={['Axe', 'Score']}
        lignes={(data ?? []).map((d: any) => [d.subject, d.A ?? d.valeur ?? 0])}
      />
    </div>
  );
};
...
export const TendanceMensuelle = ({ data }: { data: any[] }) => {
  ...
  return (
    <div className="h-72 rounded-2xl border border-border/70 bg-card p-5 shadow-premium">
      ...
      <p className="mb-3 text-xs text-muted-foreground">
        {data.length >= 2 ? (
          (() => {
            ...
            if (delta > 0) return <span className="font-semibold text-success">↗ En hausse de {delta} pt ce mois-ci ({dernier.nb_avis} avis)</span>;
            if (delta < 0) return <span className="font-semibold text-destructive">↘ En baisse de {Math.abs(delta)} pt ce mois-ci ({dernier.nb_avis} avis)</span>;
      ...
      <div
        role="img"
        aria-label={`Tendance mensuelle du score moyen sur 5 : ${data
          .map((d: any) => `${d.mois} ${d.score_moyen}/5 (${d.nb_avis} avis)`)
          .join(', ')}.`}
      >
        <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
          <AreaChart data={data} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
            ...
            <Tooltip
              contentStyle={{ borderRadius: 12, border: '1px solid hsl(var(--border))' }}
              formatter={(value: any, _name: any, item: any) => [`${value}/5`, `Score moyen (${item?.payload?.nb_avis ?? 0} avis)`]}
            />
            <Legend />
            <Area
              type="monotone"
              ...
              dot={(props: any) => {
                ...
                const fill =
                  payload.score_moyen >= 4 ? 'hsl(var(--success))'
      ...
      <TableauAccessible
        legende="Score moyen par mois, sur 5"
        entetes={['Mois', 'Score moyen sur 5', "Nombre d'avis"]}
        lignes={data.map((d: any) => [d.mois, d.score_moyen, d.nb_avis])}
      />
    </div>
  );
};
...
export const ClassementGuichets = ({ data }: { data: any[] }) => {
  ...
  return (
    <div className="rounded-2xl border border-border/70 bg-card p-5 shadow-premium" style={{ height: hauteur }}>
      ...
      <div
        role="img"
        aria-label={`Classement des guichets, du plus faible au plus performant : ${data
      ...
      <TableauAccessible
        legende="Classement des guichets par score moyen"
        entetes={['Guichet', 'Score moyen sur 5', "Nombre d'avis"]}
        lignes={data.map((d) => [d.nom, d.score_moyen, d.nb_avis ?? 0])}
      />
    </div>
  );
};
...
export const ComparaisonAgents = ({ data }: { data: any[] }) => {
  ...
  return (
    <div className="h-64 rounded-2xl border border-border/70 bg-card p-5 shadow-premium">
      ...
      <div
        role="img"
        aria-label={`Scores de satisfaction par agent : ${data
      ...
      <TableauAccessible
        legende="Scores de satisfaction par agent"
        entetes={['Agent', 'Score moyen sur 5']}
        lignes={data.map((d) => [d.nom, d.score_moyen])}
      />
    </div>
  );
};
