...
// --- Contexte d'entités factice ---------------------------------------------
// Trace les appels pour pouvoir AFFIRMER ce qui a été demandé à la base,
// pas seulement ce qui a été renvoyé : une requête sans filtre tenant
// passe inaperçue si on ne regarde que la valeur de retour.
type Trace = { appels: Array<{ modele: string; methode: string; where?: any }> };
...
function creerContexte(options: {
  critereId?: number;
  scoreOfficiel?: number | null;
  lignesExistantes?: any[];
} = {}) {
  const trace: Trace = { appels: [] };
  const critereId = options.critereId ?? 1;
  const note = (modele: string) => ({
    [modele]: new Proxy({} as Record<string, any>, {
      get: (cible: any, methode: string) => {
        // Une propriété assignée directement (surcharge dans un test)
        // doit l'emporter sur le comportement générique.
        if (Object.prototype.hasOwnProperty.call(cible, methode)) return cible[methode];
  ...
  (prisma as any).$transaction = async (fn: any) => {
    ...
    const tx: any = { $executeRaw: async () => 0 };
  ...
  return { context: { entities, req: { ip: ipSuivante() } } as any, trace };
}
...
describe('Surface publique — identification par le code opaque', () => {
  test('un code absent est refusé 400, avec un message qui dit quoi faire', async () => {
    ...
    const r = await refus(soumettreAvis({ responses: [{ critereId: 1, score: 4 }] } as any, context));
  ...
  test('un code inconnu est refusé 404, pas 500', async () => {
    const { context } = creerContexte();
    (context.entities.Guichet.findUnique as any) = async () => null;
    const r = await refus(soumettreAvis({
...
describe('Surface publique — périmètre tenant', () => {
  test('la lecture des critères est filtrée par entreprise', async () => {
    ...
    const lectureCriteres = trace.appels.find(
      (a) => a.modele === 'Critere' && a.methode === 'findMany',
    );
  ...
  test('le code_public est le seul identifiant transmis au serveur', async () => {
    const { context } = creerContexte();
    const resultat: any = await soumettreAvis({
      code_public: CODE_VALIDE,
      responses: [{ critereId: 1, score: 4 }],
    } as any, context);
...
describe('Surface publique — T2 (compléter une soumission)', () => {
  test('un identifiant de soumission vide est refusé 400', async () => {
    const { context } = creerContexte();
    const r = await refus(completerSoumission({ id_soumission: '', commentaire: 'bonjour' } as any, context));
    expect(r.statusCode).toBe(400);
  });

  test('sans commentaire ni téléphone, il n\'y a rien à enregistrer', async () => {
    const { context } = creerContexte();
    const r = await refus(completerSoumission({ id_soumission: 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee' } as any, context));
    expect(r.statusCode).toBe(400);
