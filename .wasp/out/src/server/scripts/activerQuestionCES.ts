...
const DESCRIPTION =
  "Effort perçu : 1 = très facile, 7 = très difficile. Une seule question, ~3 secondes.";

export async function activerQuestionCES(
  client: PrismaClient,
  idEntreprise?: number,
): Promise<ResultatCES> {
...
if (lanceEnCLI) {
  ...
  activerQuestionCES(prisma, Number.isInteger(id) ? id : undefined)
    .then((r) => {
      ...
      console.log(
        `[activerQuestionCES] entreprise=${r.entreprise} critere_id=${r.critereId} ` +
