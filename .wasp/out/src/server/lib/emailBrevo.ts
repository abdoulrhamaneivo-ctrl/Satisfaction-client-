...
const empreinte = (valeur: string): string =>
  crypto.createHash('sha256').update(valeur).digest('hex').slice(0, 8);

export function expediteurBrevo(): { name: string; email: string } {
...
export async function envoyerEmailBrevo({ to, subject, text, html }: EmailBrevo): Promise<void> {
  const apiKey = process.env.BREVO_API_KEY?.trim();
  if (!apiKey) {
    throw new HttpError(
      503,
      "Envoi d'e-mail indisponible : clé API Brevo (BREVO_API_KEY) non configurée. Prévenez votre administrateur."
    );
  }
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 || res.status === 403) {
      throw new HttpError(
        503,
        "Envoi d'e-mail indisponible : clé API Brevo invalide ou expéditeur non vérifié. Prévenez votre administrateur."
      );
    }
    if (res.status === 400) {
