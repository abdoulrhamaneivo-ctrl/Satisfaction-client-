...
function estErreurRateLimit(err: any): boolean {
    ...
    try {
      ...
      if (String(err?.message ?? '').includes('reasoning')) {
        ...
        try {
          ...
          throw new Error(
            `Limite NVIDIA NIM atteinte (~40 req/min, free tier). Réessaie dans quelques secondes. Détail: ${retryErr?.message ?? err?.message}`,
          );
        }
      } else {
        throw err;
