...
export type ReponseAffichable = {
  ...
  optionsChoisies?: { option?: { libelle?: string | null } | null }[] | null;
};

/** Libellés des options choisies, dans l'ordre de la jointure. */
export function libellesOptionsChoisis(r: ReponseAffichable): string[] {
...
function noteMetier(r: ReponseAffichable): number | null {
...
export function borneEchelle(critere: ReponseAffichable['critere']): { min: number; max: number } | null {
  const [a, b] = String(critere?.options_reponse || '').split(',').map((v) => Number(String(v).trim()));
  if (!Number.isFinite(a) || !Number.isFinite(b) || !(b > a)) return null;
  return { min: a, max: b };
}

function estCES(critere: ReponseAffichable['critere']): boolean {
 ...
 */
export function libelleOuiNon(r: ReponseAffichable): 'Oui' | 'Non' | null {
 ...
 */
export function reponseEstPositive(r: ReponseAffichable): boolean | null {
  const s = noteMetier(r);
  if (s !== 1 && s !== 5) return null;
  return s === 5;
}
...
export function libelleEchelle(r: ReponseAffichable): string | null {
  ...
  if (estCES(r.critere)) {
    const labels = LIBELLES_ECHELLE_CES[bornes.max as 5 | 7];
 ...
 */
export function reponseEnClair(
  r: ReponseAffichable,
  options?: { texteGroupe?: string | null },
): string | null {
  const type = String(r.critere?.type_reponse || '').toUpperCase();
...
export function decrireReponse(
  r: ReponseAffichable,
  options?: { texteGroupe?: string | null; prefixeCritere?: boolean },
): string {
