...
import { useQuery, getCriteresParOperation, moveCritereToService, removeCritereFromService, createCritere, deleteCritere, duplicateCritere, updateCritere, } from 'wasp/client/operations';
...
export const QuestionsParOperation = ({ selectedAgenceId }) => {
    ...
    const [activeCritere, setActiveCritere] = useState(null);
    ...
    const sensors = useSensors(useSensor(MouseSensor, { activationConstraint: { distance: 6 } }), useSensor(TouchSensor, { activationConstraint: { delay: 180, tolerance: 8 } }), useSensor(KeyboardSensor, { coordinateGetter: sortableKeyboardCoordinates }));
    useEffect(() => {
        setColumns([]);
        setActiveCritere(null);
        setAddingToColumn(null);
    }, [selectedAgenceId]);
    const findColumnOfCritere = (critereId) => columns.find((c) => c.criteres.some((q) => q.id === critereId));
    const findColumnByKey = (key) => columns.find((c) => c.key === key);
    ...
    const handleAddQuestion = async (col) => {
        const libelle = nouvelleQuestion.trim();
        if (!libelle || creatingInline)
            return;
    ...
    const handleDelete = async (critere) => {
        if (deletingId)
            return;
    ...
    if (isLoading) {
        return (<div className="flex gap-4 overflow-x-auto momentum-scroll scroll-fade-x pb-2">
        {[0, 1, 2].map((i) => (<div key={i} className="h-64 w-64 shrink-0 animate-pulse rounded-2xl border border-border/70 bg-card-subtle/50"/>))}
      </div>);
    }
    if (error) {
        return (<div className="rounded-xl border border-destructive/20 bg-destructive/10 p-4 text-sm text-destructive">
        Erreur de chargement des questions par opération.
      </div>);
    }
    return (<>
    <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragCancel={() => {
            setIsDragging(false);
            setActiveCritere(null);
        }} 
    ...
    autoScroll={{ threshold: { x: 0.2, y: 0.2 } }}>
      ...
      <div className={`-mx-1 flex gap-5 overflow-x-auto momentum-scroll scroll-fade-x px-1 pb-4 snap-x snap-proximity transition-opacity ${isSaving ? 'pointer-events-none opacity-60' : ''}`}>
        {columns.map((col) => (<ColumnView key={col.key} column={col} columns={columns} onMoveTo={moveCritereTo} addingToColumn={addingToColumn} setAddingToColumn={setAddingToColumn} nouvelleQuestion={nouvelleQuestion} setNouvelleQuestion={setNouvelleQuestion} nouveauType={nouveauType} setNouveauType={setNouveauType} onAddQuestion={handleAddQuestion} creatingInline={creatingInline} onDelete={handleDelete} onDuplicate={handleDuplicate} onEdit={openEdit} deletingId={deletingId} duplicatingId={duplicatingId}/>))}
      </div>
      ...
      <DialogContent className="sm:max-w-md rounded-2xl">
        ...
        <div className="space-y-4 py-2">
          <div className="space-y-1.5">
            ...
            <Input value={editLibelle} onChange={(e) => setEditLibelle(e.target.value)} maxLength={300} placeholder="Ex. : L'accueil a-t-il été rapide ?"/>
          </div>
          <div className="space-y-1.5">
            ...
            <Select value={editType} onValueChange={setEditType}>
              ...
              <SelectContent>
                {allTypeReponseOptions.map((opt) => (<SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
          {(editType === 'QCM' || editType === 'CASES' || editType === 'ECHELLE') && (<div className="space-y-1.5">
              <label className="text-xs font-semibold text-muted-foreground">
                {editType === 'ECHELLE' ? 'Plage de l’échelle (min,max)' : 'Choix proposés (séparés par des virgules)'}
              </label>
              <Input value={editOptions} onChange={(e) => setEditOptions(e.target.value)} placeholder={editType === 'ECHELLE' ? '1,5' : 'Rapide, Correct, Lent'}/>
...
function ColumnView({ column, columns, onMoveTo, addingToColumn, setAddingToColumn, nouvelleQuestion, setNouvelleQuestion, nouveauType, setNouveauType, onAddQuestion, creatingInline, onDelete, onDuplicate, onEdit, deletingId, duplicatingId, }) {
    const { setNodeRef, isOver } = useDroppable({ id: column.key });
    const isAdding = addingToColumn === column.key;
    return (<div className="flex w-[85vw] shrink-0 snap-start flex-col rounded-2xl border border-border/70 bg-card-subtle/40 shadow-sm sm:w-[22rem] xl:w-[24rem]">
      ...
      {isAdding && (<div className="space-y-1.5 border-b border-border/60 bg-background/60 px-3 py-2">
          <div className="flex items-center gap-1.5">
            <Input autoFocus value={nouvelleQuestion} onChange={(e) => setNouvelleQuestion(e.target.value)} onKeyDown={(e) => {
                if (e.key === 'Enter')
                    onAddQuestion(column);
                if (e.key === 'Escape')
                    setAddingToColumn(null);
      ...
      <div ref={setNodeRef} className={`flex min-h-[120px] flex-1 flex-col gap-2 p-3 transition-colors ${isOver ? 'bg-primary/5' : ''}`}>
        <SortableContext items={column.criteres.map((q) => q.id)} strategy={verticalListSortingStrategy}>
          {column.criteres.length === 0 && (<p className="py-6 text-center text-xs text-muted-foreground">
              Glissez une question ici
            </p>)}
          {column.criteres.map((q) => (<SortableQuestionCard key={q.id} critere={q} columns={columns} currentKey={column.key} onMoveTo={onMoveTo} onDelete={onDelete} onDuplicate={onDuplicate} onEdit={onEdit} isDeleting={deletingId === q.id} isDuplicating={duplicatingId === q.id}/>))}
        </SortableContext>
      </div>
    </div>);
}
function SortableQuestionCard({ critere, columns, currentKey, onMoveTo, onDelete, onDuplicate, onEdit, isDeleting, isDuplicating, }) {
    ...
    return (<div ref={setNodeRef} style={style}>
      <QuestionCard critere={critere} dragHandleProps={{ ...attributes, ...listeners }} columns={columns} currentKey={currentKey} onMoveTo={onMoveTo} onDelete={onDelete} onDuplicate={onDuplicate} onEdit={onEdit} isDeleting={isDeleting} isDuplicating={isDuplicating}/>
    </div>);
}
function QuestionCard({ critere, dragHandleProps, dragging, columns, currentKey, onMoveTo, onDelete, onDuplicate, onEdit, isDeleting, isDuplicating, }) {
    return (<div className={`group flex items-start gap-2 rounded-xl border border-border/70 bg-card p-3 shadow-sm ${dragging ? 'shadow-lg ring-2 ring-primary/40' : ''}`}>
      ...
      <Button type="button" variant="ghost" size="icon" className="-ml-1 -mt-1 size-9 shrink-0 cursor-grab touch-none active:cursor-grabbing" {...dragHandleProps} aria-label="Réordonner">
      ...
      <div className="min-w-0 flex-1 pt-1">
        ...
        <div className="mt-1 flex items-center gap-1.5 flex-wrap">
          ...
          {critere.actif === false && (<span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-semibold text-muted-foreground">
              Désactivé
            </span>)}
        </div>
        ...
        {columns && currentKey && onMoveTo && (<Select value={currentKey} onValueChange={(v) => onMoveTo(critere.id, v)}>
            <SelectTrigger className="mt-2 h-7 w-full text-xs font-medium" onClick={(e) => e.stopPropagation()} aria-label={`Déplacer « ${critere.libelle_critere} » vers une autre opération`}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {columns.map((c) => (<SelectItem key={c.key} value={c.key}>
                  {c.key === currentKey ? `📍 ${c.title}` : `Déplacer vers : ${c.title}`}
                </SelectItem>))}
            </SelectContent>
          </Select>)}

        {(onEdit || onDelete || onDuplicate) && (<div className="mt-1.5 flex items-center gap-1 flex-wrap">
...
//# sourceMappingURL=QuestionsParOperation.jsx.map