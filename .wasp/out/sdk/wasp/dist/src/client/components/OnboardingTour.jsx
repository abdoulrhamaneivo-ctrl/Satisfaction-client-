...
export function OnboardingTour() {
    ...
    useEffect(() => {
        ...
        const focusables = () => Array.from(node?.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])') ?? []).filter((el) => !el.hasAttribute('disabled') && el.offsetParent !== null);
        focusables()[0]?.focus();
    ...
    return (<AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/75 " onClick={handleComplete}/>

        {/* Modal / Bulle du tutoriel pas-à-pas */}
        <motion.div key={currentStepIndex} ref={dialogueRef} role="dialog" aria-modal="true" aria-labelledby="tour-titre" aria-describedby="tour-description" initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} transition={{ duration: 0.2 }} className="relative z-10 w-full max-w-lg rounded-3xl border border-primary/40 bg-card p-6 sm:p-8 shadow-2xl ring-1 ring-white/10">
          {/* Bouton Fermer */}
          <button onClick={handleComplete} className="absolute right-5 top-5 rounded-full p-2 text-muted-foreground hover:bg-muted hover:text-foreground transition-colors" title="Passer le tutoriel">
            <X className="size-4"/>
          </button>
          ...
          <div className="flex items-center justify-between border-t border-border/80 pt-5">
            <div className="flex items-center gap-1.5">
              {DEFAULT_STEPS.map((_, idx) => (<span key={idx} className={`h-2 rounded-full transition-all duration-300 ${idx === currentStepIndex
                ? 'w-6 bg-primary'
            ...
            <div className="flex items-center gap-2">
              {!isFirst && (<Button size="sm" intent="outline" onClick={handlePrev} className="rounded-xl font-bold gap-1 text-xs">
                  <ArrowLeft className="size-3.5"/> Précédent
                </Button>)}
              <Button size="sm" intent="primary" onClick={handleNext} className="rounded-xl font-bold gap-1 text-xs bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm">
                {isLast ? (<>
...
export function TriggerOnboardingButton() {
    return (<button onClick={() => window.dispatchEvent(new Event('yeba:start-tour'))} className="flex items-center gap-2 text-xs font-bold text-muted-foreground hover:text-primary transition-colors p-2 rounded-xl hover:bg-muted/50 w-full" title="Lancer le tutoriel guidé">
      <HelpCircle className="size-4 text-primary"/>
      <span>Tutoriel guidé</span>
    </button>);
}
//# sourceMappingURL=OnboardingTour.jsx.map