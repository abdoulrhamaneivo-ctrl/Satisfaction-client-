...
export function CommandPalette() {
    ...
    const { data: resultats, isLoading: chargement } = useQuery(getRechercheGlobale, { q: requeteDebattue }, { enabled: rechercheActive });
    ...
    const actionsFiltrees = useMemo(() => {
        ...
        const estDirectionPure = user?.role === 'DIRECTION' && user?.id_agence == null;
        return ACTIONS_NAVIGATION.filter((a) => {
            if (a.roles && !a.roles.includes(user?.role ?? ''))
                return false;
            // Direction pure : pas d'entrée Avis (RG16/RG17, comme la Sidebar).
            if (a.id === 'nav-avis' && estDirectionPure)
                return false;
            if (q.length === 0)
                return true;
            return a.label.toLowerCase().includes(q);
        });
    }, [requete, user?.role, user?.id_agence]);
    ...
    const groupes = useMemo(() => {
        ...
        if (rechercheActive && resultats) {
            ...
            if (resultats.guichets?.length > 0) {
                liste.push({
                    titre: 'Guichets',
                    items: resultats.guichets.map((g) => ({
                        id: `guichet-${g.id}`,
                        label: g.nom_guichet,
    ...
    return (<div className="fixed inset-0 z-[100] flex items-start justify-center pt-[12vh]" role="dialog" aria-modal="true" aria-label="Recherche globale">
      ...
      <div className="relative z-10 w-full max-w-xl overflow-hidden rounded-2xl border border-border/70 bg-card shadow-2xl">
        <div className="flex items-center gap-3 border-b border-border/70 px-4 py-3">
          ...
          <Input ref={inputRef} value={requete} onChange={(e) => setRequete(e.target.value)} onKeyDown={onKeyDownListe} placeholder="Rechercher une page, une agence, un guichet, un agent, un avis…" className="h-auto w-full border-0 bg-transparent p-0 shadow-none focus-visible:ring-0"/>
        ...
        <div className="max-h-[60vh] overflow-y-auto py-2">
          {groupes.length === 0 && (<div className="px-4 py-8 text-center text-sm text-muted-foreground">
              {rechercheActive ? 'Aucun résultat.' : 'Tapez au moins 2 caractères pour chercher dans les avis, agences, guichets et agents.'}
            </div>)}

          {groupes.map((groupe) => (<div key={groupe.titre} className="mb-1 last:mb-0">
              ...
              {groupe.items.map((item) => {
                ...
                return (<Button key={item.id} type="button" variant="ghost" onMouseEnter={() => setIndexActif(compteurGlobal)} onClick={() => selectionner(item)} className={cn('h-auto w-full justify-start gap-3 rounded-none px-4 py-2.5 text-left text-sm font-normal', actif ? 'bg-primary/10 text-foreground' : 'text-foreground hover:bg-muted/60')}>
...
export function CommandPaletteTrigger() {
    ...
    return (<Button type="button" variant="outline" onClick={() => window.dispatchEvent(new Event('yeba:open-command-palette'))} className="h-auto gap-2 rounded-full border-border/70 bg-card-subtle/60 px-3 py-1.5 text-xs font-normal text-muted-foreground hover:bg-muted/60">
