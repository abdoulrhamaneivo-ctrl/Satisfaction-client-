...
function intensiteVersOpacite(nb, maxNb) {
    if (maxNb <= 0 || nb <= 0)
        return 0;
...
export const HeatmapReponses = ({ data, isLoading }) => {
    ...
    if (!data || data.total_avis === 0) {
        return (<div className="rounded-2xl border border-border/70 bg-card p-5 shadow-premium">
    ...
    return (<div className="rounded-2xl border border-border/70 bg-card p-5 shadow-premium">
      <div className="mb-4 flex flex-wrap items-baseline justify-between gap-2">
        ...
        {survol && (<div className="rounded-lg border border-border/70 bg-card-subtle/80 px-3 py-1.5 text-xs">
      ...
      <div className="overflow-x-auto">
        ...
        <div className="min-w-[820px]" aria-hidden="true">
          ...
          <div className="grid grid-cols-[3rem_repeat(24,minmax(0,1fr))] gap-1">
            <div />
            {Array.from({ length: 24 }, (_, h) => (<div key={h} className="text-center text-[10px] text-muted-foreground">
                {h % 3 === 0 ? h : ''}
              </div>))}
          </div>
          ...
          <div className="mt-1 flex flex-col gap-1">
            {ORDRE_JOURS.map((jour) => (<div key={jour} className="grid grid-cols-[3rem_repeat(24,minmax(0,1fr))] items-center gap-1">
                ...
                return (<button key={`${cellule.jour}-${cellule.heure}`} type="button" tabIndex={-1} onMouseEnter={() => setSurvol(cellule)} onMouseLeave={() => setSurvol((s) => (s === cellule ? null : s))} className="aspect-square w-full rounded-[4px] border border-border/40 transition-transform hover:scale-110 hover:z-10" style={{
                        backgroundColor: cellule.nb > 0 && couleur
                            ? couleur
                            : 'var(--muted, #E5E7EB)',
      ...
      <table className="sr-only">
        ...
        <thead>
          <tr>
            ...
            {Array.from({ length: 24 }, (_, h) => (<th key={h} scope="col">
                {h}h
              </th>))}
          </tr>
        </thead>
...
//# sourceMappingURL=HeatmapReponses.jsx.map