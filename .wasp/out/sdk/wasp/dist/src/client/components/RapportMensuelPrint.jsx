...
export const RapportMensuelPrint = React.forwardRef((props, ref) => {
    const { reponses, radarData, alertes, taches, themes, guichets, agenceName, commune, entrepriseName, periodeLabel, dateDebut, dateFin, deltas, tempsTraitement, ces, } = props;
    ...
    const distribution = [1, 2, 3, 4, 5].map((note) => {
        const nb = avisNotes.filter((a) => Math.round(a.score_moyen) === note).length;
        return { note, nb, pct: totalNotes > 0 ? Math.round((nb / totalNotes) * 100) : 0 };
    });
    const noteMoyenne = totalNotes > 0 ? avisNotes.reduce((s, a) => s + a.score_moyen, 0) / totalNotes : 0;
    const tauxSatisfaction = totalNotes > 0 ? (avisNotes.filter((a) => a.score_moyen >= 4).length / totalNotes) * 100 : 0;
...
//# sourceMappingURL=RapportMensuelPrint.jsx.map