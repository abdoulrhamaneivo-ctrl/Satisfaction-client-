...
export const GuichetsPage = () => {
    ...
    const [editingGuichetId, setEditingGuichetId] = useState(null);
    ...
    const [guichetAConfirmer, setGuichetAConfirmer] = useState(null);
    const [guichetAArchiver, setGuichetAArchiver] = useState(null);
    ...
    const effectiveAgenceId = isDirection ? (selectedAgenceId ?? userAgenceId ?? undefined) : (userAgenceId || undefined);
    const { data: guichets, isLoading, error: queryError, refetch: refetchGuichets, } = useQuery(getGuichets, { id_agence: effectiveAgenceId || 0 }, { enabled: !!effectiveAgenceId });
    ...
    const startEditingServices = (g) => {
        setEditingGuichetId(g.id);
        setEditServiceIds(g.services?.map((s) => s.id) || []);
    };
    const handleSaveServices = async (guichetId) => {
    ...
    const guichetsFiltres = (guichets ?? []).filter((guichet) => {
        if (!rechercheNormalisee)
            return true;
        const operations = (guichet.services ?? []).map((service) => service.libelle_service).join(' ');
    ...
    return (<RequireEnterpriseRole>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
            ...
            {user?.role === 'CHEF_AGENCE' || isDirection ? (<Reveal delay={0.05}>
                <Card variant="feature" className="h-fit p-6 lg:sticky lg:top-8 rounded-3xl">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    {error && (<div className="flex items-start gap-2.5 rounded-2xl border border-destructive/25 bg-destructive/10 p-3.5 text-xs font-bold text-destructive">
                        <AlertCircle className="mt-0.5 size-4 shrink-0"/>
                        <span>{error}</span>
                      </div>)}

                    <FormField label="Nom du Guichet / Caisse" htmlFor="nom-guichet" required>
                      <Input id="nom-guichet" required value={nomGuichet} onChange={(e) => setNomGuichet(e.target.value)} placeholder="Ex: Caisse 1, Guichet Accueil..." className="h-11 rounded-2xl border-border/80"/>
                    </FormField>
                    ...
                    <FormField label="Type de guichet">
                      <Select value={typeGuichet} onValueChange={setTypeGuichet}>
                        ...
                        <SelectContent className="rounded-2xl border-border/80">
                          {TYPES_GUICHET.map((t) => (<SelectItem key={t.value} value={t.value}>
                              {t.label}
                            </SelectItem>))}
                        </SelectContent>
                      </Select>
                    ...
                    <FormField label="Opérations gérées par ce guichet">
                      ...
                      <div className="space-y-2 rounded-2xl border border-border/70 p-3.5 bg-muted/30">
                        {allServices?.map((s) => (<label key={s.id} className="flex items-center gap-2.5 text-xs font-bold text-foreground cursor-pointer">
                            <Checkbox checked={selectedServiceIds.includes(s.id)} onCheckedChange={(checked) => {
                    if (checked) {
                        ...
                        {(!allServices || allServices.length === 0) && (<p className="text-xs text-muted-foreground">Aucune opération créée pour le moment.</p>)}
                        <div className="flex gap-2 pt-2">
                          <Input value={newServiceName} onChange={(e) => setNewServiceName(e.target.value)} placeholder="Nouvelle opération" className="h-9 text-xs rounded-xl border-border/80" onKeyDown={(e) => {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    handleCreateService();
                }
            }}/>
                          <Button type="button" variant="outline" disabled={creatingService || !newServiceName.trim()} onClick={handleCreateService} className="h-9 shrink-0 px-3 text-xs rounded-xl font-bold border-border/80">
            ...
            <div className="space-y-6 lg:col-span-2">
              ...
              {isLoading && (<div className="space-y-4">
                  {[0, 1].map((i) => (<div key={i} className="h-40 animate-pulse rounded-3xl border border-border/70 bg-card/50"/>))}
                </div>)}

              {queryError && (<div className="flex items-center justify-between gap-3 rounded-2xl border border-destructive/25 bg-destructive/10 p-4 text-sm font-bold text-destructive">
                  <span>Impossible de charger vos guichets. Vérifiez votre connexion.</span>
                  <Button type="button" size="sm" variant="outline" onClick={() => refetchGuichets()} className="shrink-0 border-destructive/30 text-destructive hover:bg-destructive/10 rounded-xl">
                    Réessayer
                  </Button>
                </div>)}

              {!isLoading && guichetCount === 0 && (<EmptyState icon={Inbox} title="Aucun guichet créé pour le moment" description="Créez votre première caisse à gauche pour générer son kit de collecte (QR Code + USSD)."/>)}
                ...
                {guichetsFiltres.map((g, i) => (<Reveal key={g.id} delay={Math.min(i * 0.05, 0.2)}>
                    <Card variant="feature" className="p-6 rounded-3xl">
                      <div className="flex flex-col items-start justify-between gap-6 md:flex-row border-b border-border/60 pb-5 mb-5">
                        <div>
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            {g.services && g.services.length > 0 && (<span className="inline-block rounded-full bg-primary/15 border border-primary/30 px-2.5 py-0.5 text-[11px] font-bold text-primary">
                      ...
                      <div className="bg-muted/30 p-4 rounded-2xl border border-dashed border-border/60">
                        {editingGuichetId === g.id ? (<div className="space-y-3">
                            ...
                            <div className="grid grid-cols-1 gap-2 py-1 sm:grid-cols-2">
                              {allServices?.map((s) => (<label key={s.id} className="flex items-center gap-2 text-xs font-bold text-foreground cursor-pointer">
                        ...
                        if (checked) {
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-1.5 mt-1.5">
                                {g.services && g.services.length > 0 ? (g.services.map((s) => (<span key={s.id} className="bg-card border border-border/80 text-[11px] font-bold text-foreground px-2.5 py-0.5 rounded-lg">
                                      {s.libelle_service}
                                    </span>))) : (<span className="text-xs text-muted-foreground italic font-medium">
      ...
      <AlertDialog open={guichetAArchiver !== null} onOpenChange={(open) => !open && setGuichetAArchiver(null)}>
        <AlertDialogContent className="rounded-3xl border-border/80">
          <AlertDialogHeader>
...
//# sourceMappingURL=GuichetsPage.jsx.map