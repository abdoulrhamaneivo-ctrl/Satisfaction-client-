...
import { AlertTriangle, CheckCircle2, Clock, PlayCircle, PlusCircle, X, Bell, Inbox, ChevronRight, History, ArrowRight, Search, } from 'lucide-react';
...
export const AlertesTachesPage = () => {
    ...
    const [filtreTaches, setFiltreTaches] = useState('TOUTES');
    ...
    const { data: responsablesBruts } = useQuery(getAgentsByAgence, { id_agence: modal.idAgence ?? 0 }, { enabled: modal.idAgence !== null });
    ...
    const responsablesPossibles = (responsablesBruts || []).filter((a) => a.actif !== false);
    const alertesList = alertes || [];
    const tachesList = taches || [];
    const rolesGestion = ['DIRECTION', 'CHEF_AGENCE'];
    const peutGererAlertes = !!currentUser && rolesGestion.includes(currentUser.role);
    ...
    const peutAgirSurTache = (tache) => !!currentUser && (rolesGestion.includes(currentUser.role) || tache.id_responsable === currentUser.id);
    ...
    useEffect(() => {
        if (!loadingTaches && taches !== undefined)
            setADejaChargeUneFois(true);
    ...
    const tachesEnRetardCount = tachesList.filter((t) => t.statut_tache !== 'TERMINEE' && new Date(t.date_echeance) < new Date()).length;
    const rechercheNormalisee = recherche.trim().toLocaleLowerCase('fr-FR');
    const correspondRecherche = (valeurs) => !rechercheNormalisee || valeurs
        .filter((valeur) => valeur !== null && valeur !== undefined)
        .join(' ')
        .toLocaleLowerCase('fr-FR')
        .includes(rechercheNormalisee);
    const alertesNouvellesFiltrees = alertesNouvelles.filter((alerte) => correspondRecherche([alerte.message, alerte.guichet?.nom_guichet, alerte.type_alerte]));
    const tachesFiltrees = tachesList.filter((tache) => {
        const estEnRetard = tache.statut_tache !== 'TERMINEE' && new Date(tache.date_echeance) < new Date();
        const estAMoi = tache.id_responsable === currentUser?.id;
    ...
    const handleSoumettreCreation = async (e) => {
        e.preventDefault();
    ...
    const handleMoveStatut = async (tacheId, statut) => {
        setMovingId(tacheId);
        try {
    ...
    const handleMarquerTraitee = async (alerteId) => {
        try {
    ...
    return (<RequireEnterpriseRole>
      ...
      <AmbientBackground>
        <div className="mx-auto max-w-7xl p-6 lg:p-10 space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/70 pb-4">
            <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground">
              <span>Agences</span>
              <span>/</span>
              <span className="text-foreground">{currentUser?.agence?.nom_agence || "Agence Principale"}</span>
              <span>/</span>
          ...
          <PageHeader icon={AlertTriangle} eyebrow="Surveillance & Amélioration" title="Alertes & Tâches correctives" description="Suivez les alertes critiques et gérez les actions correctives associées en mode Kanban."/>
        ...
        <section>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-title-sm font-bold text-foreground flex items-center gap-2">
              ...
              Alertes nouvelles
              {alertesNouvelles.length > 0 && (<span className="ml-1 rounded-full bg-destructive/10 px-2.5 py-0.5 text-xs font-bold text-destructive">
          ...
          {loadingAlertes ? (<div className="space-y-3">
              ...
              {alertesNouvellesFiltrees.length === 0 ? (<p className="rounded-xl border border-dashed border-border/70 px-4 py-6 text-center text-sm text-muted-foreground">
                  Aucune alerte ne correspond à votre recherche.
                </p>) : alertesNouvellesFiltrees.map((alerte, i) => (<motion.div key={alerte.id.toString()} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}>
        ...
        <section>
          <h2 className="mb-4 text-title-sm font-bold text-foreground flex items-center gap-2">
            ...
            {tachesEnRetardCount > 0 && (<span className="rounded-full bg-destructive/10 px-2.5 py-0.5 text-xs font-bold text-destructive flex items-center gap-1">
                <AlertTriangle className="size-3"/> {tachesEnRetardCount} en retard
              </span>)}
          </h2>

          {afficherSquelette ? (<div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
              {[0, 1, 2].map((i) => (<div key={i} className="h-64 animate-pulse rounded-2xl border border-border/70 bg-card-subtle/50"/>))}
            </div>) : (<div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
              {COLONNES.map((col) => {
                const tachesColonne = tachesFiltrees.filter((t) => t.statut_tache === col.statut);
                return (<div key={col.statut} className="rounded-2xl border border-border/70 bg-card/50 p-4">
                    ...
                    <div className="space-y-3">
                      <AnimatePresence>
                        ...
                        return (<motion.div key={tache.id.toString()} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.9 }}>
                              <MotionCard className={`p-4 space-y-2.5 ${col.statut !== 'TERMINEE' && new Date(tache.date_echeance) < new Date()
                                ? 'border-destructive/40 bg-destructive/5'
                                : ''}`}>
                                <p className="text-sm font-semibold text-foreground leading-snug">{tache.titre}</p>
                                {tache.description && (<p className="text-xs text-muted-foreground line-clamp-2">{tache.description}</p>)}
                                <div className="flex items-center justify-between text-xs text-muted-foreground">
                                  <span className={col.statut !== 'TERMINEE' && new Date(tache.date_echeance) < new Date()
                                ? 'flex items-center gap-1 font-bold text-destructive'
                                : ''}>
                                    {col.statut !== 'TERMINEE' && new Date(tache.date_echeance) < new Date() && (<AlertTriangle className="size-3"/>)}
                                  ...
                                  {tache.responsable && (<span className="font-medium text-foreground">
                                      {tache.responsable.prenom} {tache.responsable.nom}
                                    </span>)}
                                </div>
                                {tache.alerte?.guichet && (<p className="text-[10px] text-muted-foreground italic">
                                    Guichet: {tache.alerte.guichet.nom_guichet}
                                  </p>)}
                                {/* Boutons de transition + historique */}
                                <div className="flex gap-2 pt-1 flex-wrap">
                                  {col.statut !== 'A_FAIRE' && peutAgirSurTache(tache) && (<Button type="button" variant="ghost" size="sm" className="h-7 px-2.5 text-[11px]" onClick={() => handleMoveStatut(tacheIdNum, col.statut === 'EN_COURS' ? 'A_FAIRE' : 'EN_COURS')} disabled={movingId === tacheIdNum}>
                                      ← Reculer
                                    </Button>)}
                                  <Button type="button" variant="ghost" size="sm" className="h-7 px-2.5 text-[11px]" onClick={() => setHistoriqueOpenId(historiqueOpenId === tacheIdNum ? null : tacheIdNum)} title="Voir l'historique d'audit">
                                    <History className="size-3"/> Historique
                                  </Button>
                                  {col.statut !== 'TERMINEE' && peutAgirSurTache(tache) && (<Button type="button" variant="ghost" size="sm" className="ml-auto h-7 px-2.5 text-[11px] text-primary hover:bg-primary/10 hover:text-primary" onClick={() => handleMoveStatut(tacheIdNum, col.statut === 'A_FAIRE' ? 'EN_COURS' : 'TERMINEE')} disabled={movingId === tacheIdNum}>
                                ...
                                <AnimatePresence>
                                  {historiqueOpenId === tacheIdNum && (<TacheHistoriquePanel idTache={tacheIdNum}/>)}
                                </AnimatePresence>
                              </MotionCard>
                            </motion.div>);
                    })}
                      </AnimatePresence>
                      {tachesColonne.length === 0 && (<p className="text-center text-xs text-muted-foreground py-6 italic">Aucune tâche</p>)}
                    </div>
                  </div>);
            })}
            </div>)}
      ...
      <AnimatePresence>
        {modal.alerteId !== null && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={(e) => e.target === e.currentTarget && setModal({ alerteId: null, idAgence: null })}>
            <motion.div initial={{ scale: 0.93, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.93, opacity: 0 }} className="w-full max-w-md rounded-2xl border border-border bg-background p-6 shadow-premium">
              <div className="mb-5 flex items-center justify-between">
                <h3 className="text-lg font-bold text-foreground">Nouvelle tâche corrective</h3>
                <Button type="button" variant="ghost" size="icon" onClick={() => setModal({ alerteId: null, idAgence: null })} aria-label="Fermer">
                  <X className="size-5"/>
                </Button>
              </div>
              ...
              <form onSubmit={handleSoumettreCreation} className="space-y-4">
                <div>
                  ...
                  <Input required value={formTache.titre} onChange={(e) => setFormTache((p) => ({ ...p, titre: e.target.value }))} placeholder="Décrire l'action corrective" className="h-11"/>
                </div>
                <div>
                  <label className="block text-xs font-semibold uppercase text-muted-foreground mb-1">Description</label>
                  <Textarea value={formTache.description} onChange={(e) => setFormTache((p) => ({ ...p, description: e.target.value }))} rows={3}/>
                </div>
                <div>
                  <label className="block text-xs font-semibold uppercase text-muted-foreground mb-1">Date d'échéance *</label>
                  <Input type="date" required value={formTache.date_echeance} onChange={(e) => setFormTache((p) => ({ ...p, date_echeance: e.target.value }))} className="h-11"/>
                </div>
                <div>
                  <label className="block text-xs font-semibold uppercase text-muted-foreground mb-1">Responsable *</label>
                  <Select value={formTache.id_responsable || undefined} onValueChange={(value) => setFormTache((p) => ({ ...p, id_responsable: value }))} disabled={responsablesPossibles.length === 0}>
                    <SelectTrigger className="h-11 w-full">
                      <SelectValue placeholder={responsablesPossibles.length > 0
                ? 'Sélectionner un responsable...'
                : 'Aucun agent disponible dans cette agence'}/>
                    </SelectTrigger>
                    <SelectContent>
                      {responsablesPossibles.map((agent) => (<SelectItem key={agent.id} value={String(agent.id)}>
...
function TacheHistoriquePanel({ idTache }) {
    const { data: historique, isLoading } = useQuery(getTacheHistorique, { id_tache: idTache });
    return (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
      <div className="mt-3 rounded-xl border border-border/60 bg-muted/30 p-3 space-y-2">
        ...
        {isLoading && (<div className="space-y-1.5">
            <div className="h-3 w-3/4 animate-pulse rounded bg-muted"/>
            <div className="h-3 w-1/2 animate-pulse rounded bg-muted"/>
          </div>)}

        {!isLoading && (!historique || historique.length === 0) && (<p className="text-xs text-muted-foreground italic">Aucun historique disponible.</p>)}

        {!isLoading && historique && historique.length > 0 && (<ol className="relative ml-2 border-l border-border/50 pl-4 space-y-2.5">
            {historique.map((h) => (<li key={h.id} className="relative">
                ...
                {h.commentaire && (<p className="text-[10px] italic text-muted-foreground">{h.commentaire}</p>)}
              </li>))}
          </ol>)}
      </div>
    </motion.div>);
}
//# sourceMappingURL=AlertesTachesPage.jsx.map