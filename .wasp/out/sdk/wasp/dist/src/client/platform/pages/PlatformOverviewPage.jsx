...
export function StatusChip({ status }) {
    ...
    return (<span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[10px] font-black uppercase tracking-widest ${styles[status] ?? styles.CANCELLED}`}>
...
export function PlanChip({ plan }) {
    ...
    return (<span className="inline-flex items-center rounded-full border border-primary/25 bg-primary/10 px-2.5 py-0.5 text-[10px] font-black uppercase tracking-widest text-primary">
      {labels[plan] ?? plan}
    </span>);
}
...
function PlatformOverviewInner() {
    ...
    if (isLoading || !data) {
        return (<div className="mx-auto max-w-6xl space-y-8" aria-busy="true" aria-label="Chargement de la vue d'ensemble">
        <div className="h-10 w-64 animate-pulse rounded-xl bg-muted"/>
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          {[0, 1, 2, 3].map((i) => (<div key={i} className="h-28 animate-pulse rounded-2xl border border-border/70 bg-card-subtle/50"/>))}
        </div>
    ...
    return (<div className="mx-auto max-w-6xl space-y-8">
      ...
      {/* KPI tiles */}
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {tiles.map(({ label, value, icon: Icon, tone }) => (<div key={label} className="rounded-2xl border border-border/80 bg-card p-5 shadow-sm">
      ...
      <section>
        ...
        <div className="grid gap-3">
          {data.recentes.length === 0 ? (<div className="rounded-2xl border border-dashed border-border p-10 text-center">
              ...
              <Link to="/platform/entreprises/nouvelle" className="mt-4 inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground hover:bg-primary/90">
                <Plus className="size-4"/> Créer la première entreprise
              </Link>
            </div>) : (data.recentes.map((e) => (<Link key={e.id} to={`/platform/entreprises/${e.id}`} className="hover-lift flex items-center justify-between rounded-2xl border border-border/80 bg-card p-5">
...
//# sourceMappingURL=PlatformOverviewPage.jsx.map