...
export function PlatformShell() {
    ...
    if (loadingMe) {
        return (<div className="flex min-h-screen items-center justify-center bg-[hsl(216_40%_12%)]">
        <p className="text-sm text-white/60">Chargement de la console…</p>
      </div>);
    }
    if (erreurMe || !me) {
        return (<div className="flex min-h-screen items-center justify-center bg-[hsl(216_40%_12%)] p-6">
        <div className="max-w-md rounded-2xl border border-white/10 bg-white/5 p-8 text-center">
          ...
          <button onClick={() => navigate('/dashboard')} className="mt-6 inline-flex items-center gap-2 rounded-xl border border-white/20 px-4 py-2 text-sm font-semibold text-white hover:bg-white/10">
    ...
    return (<div className="flex min-h-screen bg-[hsl(216_40%_12%)] text-foreground">
      {/* Sidebar desktop */}
      <aside className="hidden w-64 shrink-0 flex-col border-r border-white/10 bg-[hsl(216_45%_10%)] p-4 md:flex">
        ...
        <nav className="flex flex-col gap-1" aria-label="Console platform">
          {LINKS.map(({ to, label, icon: Icon, end }) => (<NavLink key={to} to={to} end={end} className={({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition-colors ${isActive ? 'bg-warning/15 text-warning' : 'text-white/70 hover:bg-white/5 hover:text-white'}`}>
              <Icon className="size-4"/> {label}
            </NavLink>))}
        </nav>
        <div className="mt-auto border-t border-white/10 pt-4">
          <button onClick={() => navigate('/dashboard')} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-white/60 hover:bg-white/5 hover:text-white">
      ...
      {menuOpen && (<div className="fixed inset-0 z-40 md:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 w-64 bg-[hsl(216_45%_10%)] p-4">
            ...
            <nav className="flex flex-col gap-1">
              {LINKS.map(({ to, label, icon: Icon, end }) => (<NavLink key={to} to={to} end={end} onClick={() => setMenuOpen(false)} className={({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold ${isActive ? 'bg-warning/15 text-warning' : 'text-white/70 hover:bg-white/5'}`}>
                  <Icon className="size-4"/> {label}
                </NavLink>))}
            </nav>
          </aside>
...
//# sourceMappingURL=PlatformShell.jsx.map