...
function CompanyDetailsInner({ id }) {
    ...
    const [limites, setLimites] = useState({ ...initialLimites, ...persistedLimites });
    ...
    const setLimitesWithPersist = (next) => {
        setLimites(next);
        // Sauvegarde locale (pas de debounce nécessaire pour les modales)
        try {
            localStorage.setItem(`form-persist:company-limites-${idEntreprise}`, JSON.stringify(next));
        }
        catch { }
    };
    if (isLoading)
    ...
    return (<div className="mx-auto max-w-5xl space-y-6">
      ...
      <header className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          {e.status === 'SUSPENDED' ? (<button onClick={async () => {
                try {
                    const r = await reactiver({ id_entreprise: e.id, totpCode });
                    setMessage(r.message);
                }
                catch (err) {
                    setErreurAction(err?.message);
                }
            }} disabled={!totpCodeValide} className="inline-flex items-center gap-2 rounded-xl bg-success px-4 py-2.5 text-sm font-bold text-white hover:bg-success/90">
              <PlayCircle className="size-4"/> Réactiver
            </button>) : (<button onClick={ouvrirSuspendre} disabled={!totpCodeValide} className="inline-flex items-center gap-2 rounded-xl border border-destructive/40 px-4 py-2.5 text-sm font-bold text-destructive hover:bg-destructive/10">
          ...
          {e.invitation_active && (<button onClick={async () => {
                if (envoiInvitationEnCours)
                    return;
                setEnvoiInvitationEnCours(true);
                try {
                    const r = await renvoyerInvitationFn({ id_entreprise: e.id, totpCode });
                    setMessage(r.message);
                }
                catch (err) {
                    setErreurAction(messageErreurAction(err, "Impossible de renvoyer l'invitation."));
                }
                finally {
                    setEnvoiInvitationEnCours(false);
                }
            }} disabled={!totpCodeValide || envoiInvitationEnCours} className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2.5 text-sm font-bold text-foreground hover:bg-muted disabled:opacity-50">
      ...
      {modalSuspendre && (<div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-md rounded-2xl bg-card p-6 shadow-xl">
            ...
            <textarea value={motif} onChange={(ev) => setMotif(ev.target.value)} rows={3} placeholder="Ex. impayé de facture, demande du client…" className="mt-1.5 w-full rounded-xl border border-border bg-background p-3 text-sm outline-none focus:ring-2 focus:ring-ring"/>
            ...
            }} disabled={motif.trim().length < 5 || !totpCodeValide} className="rounded-xl bg-destructive px-4 py-2.5 text-sm font-bold text-white hover:bg-destructive/90 disabled:opacity-50">
      ...
      {modalLimites && (<div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-md rounded-2xl bg-card p-6 shadow-xl">
            ...
            <div className="mt-4 space-y-3">
              ...
              <select value={e.plan} onChange={(ev) => { /* plan modifiable via changerLimites plan */ window.__newPlan = ev.target.value; }} className="h-10 w-full rounded-xl border border-border bg-background px-3 text-sm text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring" id="select-plan" defaultValue={e.plan}>
