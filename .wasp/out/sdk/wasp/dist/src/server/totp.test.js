...
import { chiffrerSecretTotp, dechiffrerSecretTotp, dechiffrerSecretTotpAvecStatut, genererSecretTotp, } from './totp';
...
function chiffrerFormatHistorique(secret, matiereCle) {
...
describe('C6a : chiffrement TOTP', () => {
    ...
    test('TOTP_ENCRYPTION_KEY_CURRENT est prioritaire pour le chiffrement', () => {
        ...
        process.env.JWT_SECRET = LEGACY_JWT;
        const stocke = chiffrerSecretTotp(genererSecretTotp());
        expect(dechiffrerSecretTotpAvecStatut(stocke).cleUtilisee).toBe('TOTP_ENCRYPTION_KEY_CURRENT');
    });
});
