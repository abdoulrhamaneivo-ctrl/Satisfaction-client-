...
const PROVIDERS = ['nvidiaProvider', 'deepseekProvider', 'openrouterProvider'];
const lire = (fichier) => readFileSync(join(ici, `${fichier}.ts`), 'utf-8');
describe('prompts IA : source unique (P14 g)', () => {
    test('aucun provider ne redéclare un prompt système', () => {
        for (const p of PROVIDERS) {
            ...
            expect(source, `${p} redéclare le prompt de synthèse`).not.toMatch(/const\s+PROMPT_SYNTHESE_SYSTEM\s*=/);
        }
    });
    ...
    test('les trois providers utilisent le MÊME prompt, importé', () => {
        for (const p of PROVIDERS) {
            expect(lire(p), `${p} n'importe pas le prompt canonique`).toMatch(/import\s*\{[^}]*SYSTEM_PROMPT[^}]*\}\s*from\s*'\.\/prompts'/);
        }
    });
