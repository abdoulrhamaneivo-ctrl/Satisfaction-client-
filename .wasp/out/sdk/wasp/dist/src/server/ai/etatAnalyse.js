...
export function estRejouable(ligne) {
    if (ligne.status === 'PENDING')
        return true;
    if (ligne.status === 'FAILED')
        return (ligne.attempts ?? 0) < MAX_ATTEMPTS_ANALYSE;
    return false;
}
/** Un traitement est-il resté bloqué trop longtemps ? (→ requeue) */
export function estObsolete(ligne, maintenant = new Date(), delaiMinutes = DELAI_OBSOLESCENCE_MINUTES) {
    if (ligne.status !== 'PROCESSING')
        return false;
    if (!ligne.updatedAt)
        return true; // pas d'horodatage : traitement non traçable, on récupère
    const maj = new Date(ligne.updatedAt);
    if (Number.isNaN(maj.getTime()))
        return true;
...
export function aRejouer(lignes, maintenant = new Date()) {
...
export function estRelancableManuellement(ligne) {
    return ligne.status === 'PENDING' || ligne.status === 'FAILED' || estObsolete(ligne);
}
//# sourceMappingURL=etatAnalyse.js.map