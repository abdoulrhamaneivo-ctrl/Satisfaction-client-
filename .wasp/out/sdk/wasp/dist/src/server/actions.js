 ...
 */
export function verifierVolumeReponses(responses, max = MAX_REPONSES_PAR_SOUMISSION) {
...
const soumettreAvisImpl = async (args, context) => {
    ...
    for (const entree of entrees) {
        ...
        if (!critere)
            continue; // garde-fous d'existence déjà appliqués plus haut
    ...
    for (const item of itemsToInsert) {
        ...
        if (type === 'TEXTE' || type === 'CASES') {
            ...
            const n5 = item.score_normalise !== null
                ? Math.max(1, Math.min(5, Math.round(item.score_normalise / 20)))
                : null;
            morceauxIA.push(`Q : ${libelle}\nNote : ${n5 !== null ? `${n5}/5` : '—'}`);
        }
    }
    if (commentaireFinal.length > 0)
        morceauxIA.push(`Commentaire final : ${commentaireFinal}`);
...
export const completerSoumission = async (args, context) => {
 ...
 */
export const completerSoumissionPublic = async (args, context) => {
    try {
        ...
        throw new HttpError(500, "Nous ne pouvons pas enregistrer votre commentaire pour le moment. Veuillez réessayer.");
    }
};
// ============================================================================
...
export const renvoyerInvitationAgent = async (args, context) => {
    ...
    try {
      ...
      <p style="color: #9ca3af; font-size: 12px;">Si vous avez déjà activé votre compte, ignorez cet email et connectez-vous avec votre mot de passe.</p>
        ...
        throw new HttpError(err?.statusCode ?? 502, `Nouveau lien créé, mais l'e-mail n'est pas parti (${err?.message ?? 'envoi impossible'}). Vérifiez la configuration e-mail puis cliquez « Renvoyer » une seule fois.`);
    }
    await journaliser({
        context,
...
export const demanderReinitialisation = async (args, context) => {
    ...
    if (!RESET_EMAIL_RE.test(email))
        return generique;
    const cible = await context.entities.User.findUnique({ where: { email } });
    if (!cible || cible.actif === false || !cible.email)
        return generique;
    const { lienActivation } = await import('./actionsPlatform');
    const tokenClair = crypto.randomBytes(32).toString('base64url');
    const tokenHash = crypto.createHash('sha256').update(tokenClair).digest('hex');
...
async function synchroniserOptionsCritere(tx, idCritere, entrees, provenance) {
    ...
    const propres = entrees.map((e, i) => {
        ...
        if (!libelle)
            throw new HttpError(400, `Le choix n°${i + 1} est vide.`);
        if (libelle.length > 200)
            throw new HttpError(400, `Le choix « ${libelle.slice(0, 40)} » dépasse 200 caractères.`);
...
export const createCritere = async (args, context) => {
    ...
    const orientationBrute = args.orientation === undefined || args.orientation === null || args.orientation === ''
        ? 'HIGHER_BETTER'
    ...
    const critere = await prisma.$transaction(async (tx) => {
        const created = await tx.critere.create({
            data: {
                ...
                scores_reponse: typeReponse === 'QCM' || typeReponse === 'CASES'
                    ? construireScoresAStocker(args.options_reponse?.trim() || '', undefined)
                    : null,
        ...
        if (typeReponse === 'QCM' || typeReponse === 'CASES') {
            ...
            const { csvOptions, csvScores } = await synchroniserOptionsCritere(tx, created.id, entrees, provenance);
...
export const updateCritere = async (args, context) => {
    ...
    const toucheScoring = typeReponse !== undefined ||
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        if (typeCourant === 'QCM' || typeCourant === 'CASES') {
            if (args.options !== undefined && args.options.length > 0) {
                const { csvOptions, csvScores } = await synchroniserOptionsCritere(tx, idCritere, args.options, 'EXPLICIT');
                await tx.critere.update({
                    where: { id: idCritere },
                    data: { options_reponse: csvOptions, scores_reponse: csvScores },
                });
                ...
                const { infererScoreOption } = await import('../shared/scoringQCM');
                const { csvOptions, csvScores } = await synchroniserOptionsCritere(tx, idCritere, parseOptionsCSV(optionsReponse || '').map((libelle) => ({
                    libelle,
                    score: infererScoreOption(libelle),
                })), 'INFERRED');
                await tx.critere.update({
