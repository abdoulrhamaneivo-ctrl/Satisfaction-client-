...
import { prioriserIrritants, niveauConfianceGlobal, calculerAgregats, construirePromptSynthese, derniereSemaineComplete, moisPrecedent, semaineContenant, moisContenant, recalerIrritantsSurMesures, } from './moteurGlobal';
...
describe('calculerAgregats : CES (Phase L)', () => {
    ...
    test('agrège le CES des critères marqués CES uniquement', async () => {
        const a = await calculerAgregats(dbAvec([
            ligneCES(1),
            ligneCES(2),
            ligneCES(5),
            ligneCES(7),
    ...
    test('aucun critère CES → ces = null (jamais une estimation)', async () => {
        const a = await calculerAgregats(dbAvec([{ ...ligneCES(3), critere: { type_reponse: 'ECHELLE', scoring_mode: 'NUMERIC', options_reponse: '1,10' } }]), { id_entreprise: 1, ...fenetre });
...
describe('prompt déterministe + schéma synthèse', () => {
    ...
    test('schéma synthèse : valide complet, rejette l’invention de champs', () => {
        ...
        expect(SyntheseGlobaleSchema.safeParse({ ...ok, confiance: 'CERTAIN' }).success).toBe(false);
    });
});
// ============================================================================
...
describe('P10 — recalage des irritants sur les mesures', () => {
    ...
    test('la priorité déterministe écrase celle du modèle', () => {
        const { retenus } = recalerIrritantsSurMesures([{ theme: 'Accueil', priorite: 3 }], mesures);
    ...
    test('les autres champs du modèle sont conservés', () => {
        const { retenus } = recalerIrritantsSurMesures([{ theme: 'Accueil', priorite: 3, constat: 'Phrase du modèle', confiance: 'ELEVEE' }], mesures);
    ...
    test('sans mesure, rien n\'est retenu — jamais de repli sur le modèle', () => {
        const { retenus, ecarte } = recalerIrritantsSurMesures([{ theme: 'Accueil', priorite: 42 }], []);
        expect(retenus).toEqual([]);
        expect(ecarte).toBe(1);
    });
});
//# sourceMappingURL=moteurGlobal.test.js.map