...
import { parseOptionsCSV, parseScoresCSV, normaliserLibelle, } from '../../shared/scoringQCM';
export async function backfillOptionsCriteres(client) {
    ...
    for (const c of criteres) {
        ...
        libelles.forEach((libelle, index) => {
            ...
            if (!normalise || vus.has(normalise)) {
                doublonsIgnores += 1;
                console.warn(`[backfill] critère ${c.id} (« ${c.libelle_critere} ») : doublon ignoré « ${libelle} »`);
                return;
            }
            vus.add(normalise);
            const score = scores && scores.length === libelles.length ? scores[index] : null;
            lignes.push({
                id_critere: c.id,
                libelle,
...
if (lanceEnCLI) {
    ...
    backfillOptionsCriteres(prisma)
        .then((r) => {
        console.log(`[backfillOptionsCriteres] traités=${r.traites} créées=${r.optionsCreees} doublons_ignorés=${r.doublonsIgnores}`);
    })
        .catch((e) => {
...
//# sourceMappingURL=backfillOptionsCriteres.js.map