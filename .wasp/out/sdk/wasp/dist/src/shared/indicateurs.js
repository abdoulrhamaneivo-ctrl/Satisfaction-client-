...
export const CATALOGUE_INDICATEURS = [
    ...
    { id: 'CES_MOYEN', label: 'CES moyen', description: "Effort perçu — seulement si le questionnaire contient une question d'effort (échelle 1-5 ou 1-7, 1 = très facile).", formule: 'moyenne(question CES) ou N/A', source: 'reponses', unite: '/100' },
    { id: 'CES_TOP_BOX', label: 'CES top box', description: "Part des clients ayant déclaré un effort faible (1-2 sur 5, 1-3 sur 7).", formule: '100 × n(1-2 [1-5] | 1-3 [1-7]) / n(CES)', source: 'reponses', unite: '%' },
    { id: 'CES_TAUX_EFFORT_ELEVE', label: 'Effort élevé', description: "Part des clients ayant déclaré un effort élevé (4-5 sur 5, 6-7 sur 7).", formule: '100 × n(4-5 [1-5] | 6-7 [1-7]) / n(CES)', source: 'reponses', unite: '%' },
    { id: 'CES_REPARTITION', label: 'Répartition effort', description: 'Effectifs par note d\'effort brute (1..échelle), dénominateur = n(CES).', formule: 'n(note k) / n(CES)', source: 'reponses', unite: 'nombre' },
...
export function distributionBandends(notes100) {
    ...
    for (const n of notes100) {
        if (n >= 80)
            ...
            dist.tres_insatisfaits += 1;
    }
    return dist;
}
export function mediane(notes) {
    if (notes.length === 0)
        return null;
...
export function definitionIndicateur(id) {
 ...
 */
export function indiceGlobalExperience(e) {
    const { csat } = e;
...
//# sourceMappingURL=indicateurs.js.map