...
import { resoudreReponse, resoudreChoixUnique, resoudreBinaire, resoudreNumerique, resoudreCES, resoudreNPS, resoudreCases, resoudreCasesMoyenne, resoudreTexte, agregerNPS, note5Vers100, echelleVers100, } from './scoringEngine';
...
function opt(id, libelle, score, extra = {}) {
    return {
    ...
    for (const ordre of ordres) {
        test(`ordre [${ordre.join(' | ')}]`, () => {
            ...
            for (const libelle of LIBELLES_5) {
                ...
                expect(r.score_normalise).toBeCloseTo((attenduParLibelle.get(libelle) - 1) * 25, 9);
            }
        });
    }
...
describe('§7 numérique / échelle', () => {
    const echelle10 = { orientation: 'HIGHER_BETTER', echelle_min: 1, echelle_max: 10 };
    ...
    test('orientation inverse : 8/10 → 22.22/100', () => {
        const r = resoudreNumerique({ orientation: 'LOWER_BETTER', echelle_min: 1, echelle_max: 10 }, 8);
        expect(r.score_normalise).toBeCloseTo(22.222, 2);
    });
    test('0-10 et 0-100 supportés', () => {
        expect(resoudreNumerique({ orientation: 'HIGHER_BETTER', echelle_min: 0, echelle_max: 10 }, 0)
            .score_normalise).toBe(0);
        expect(resoudreNumerique({ orientation: 'HIGHER_BETTER', echelle_min: 0, echelle_max: 100 }, 100)
            .score_normalise).toBe(100);
    });
    test('hors bornes / non entier / mal configurée → AMBIGU', () => {
        ...
        expect(resoudreNumerique({ orientation: 'HIGHER_BETTER', echelle_min: 5, echelle_max: 5 }, 5).raison).toBe('ECHELLE_MAL_CONFIGUREE');
    });
...
describe('§10 + §63 CASES', () => {
    ...
    test('« Aucun » + autre choix → AMBIGU (exclusivité)', () => {
        ...
        expect(resoudreCases(c, ['aucun', 'att'], 'INFERRED', normaliserLibelle).raison).toBe('EXCLUSIVITE_VIOLEE');
    });
...
describe('dispatcher : déduction du type', () => {
    ...
    test('QCM→ORDINAL, OUI_NON→BINARY, ECHELLE→NUMERIC, SMILEY, NPS', () => {
        ...
        expect(resoudreReponse({ ...base, type_reponse: 'OUI_NON' }, { type: 'binaire', valeurOui: true })
            .score_officiel).toBe(5);
        expect(resoudreReponse({ ...base, type_reponse: 'ECHELLE', echelle_min: 1, echelle_max: 5 }, { type: 'valeur', valeur: 4 }).score_normalise).toBeCloseTo(75, 9);
        expect(resoudreReponse({ ...base, type_reponse: 'SMILEY' }, { type: 'option', optionId: 'o' }).statut).toBe('OK');
        expect(resoudreReponse({ ...base, type_reponse: 'NPS' }, { type: 'valeur', valeur: 9 })
            .categorie_nps).toBe('PROMOTEUR');
    });
});
describe('CES : effort perçu, sens imposé', () => {
    const ces = (max, orientation = 'HIGHER_BETTER') => ({
        scoring_mode: 'CES',
