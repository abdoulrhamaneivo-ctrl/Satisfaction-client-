...
export interface ReponseCalculable {
  ...
  critere?: { type_reponse?: string | null; scoring_mode?: string | null } | null;
}

/** Sépare les lignes orphelines (sans `id_soumission`) : chacune son avis. */
export function grouperParAvis<T extends ReponseCalculable>(lignes: T[]): T[][] {
  const parSoumission = new Map<string, T[]>();
  const orphelines: T[] = [];
...
export function compterAvisDans<T extends ReponseCalculable>(lignes: T[]): number {
 ...
 * exploitable.
 */
export function scoreAvis100<T extends ReponseCalculable>(lignes: T[]): number | null {
  const notes: number[] = [];
  for (const ligne of lignes) {
    ...
    const score = noteSur5(ligne as any);
 ...
 */
export function csatParAvis<T extends ReponseCalculable>(reponses: T[]): number | null {
 ...
 */
export function scoresAvisSatisfaction<T extends ReponseCalculable>(reponses: T[]): number[] {
  const scores: number[] = [];
 ...
 */
export function distributionParAvis<T extends ReponseCalculable>(reponses: T[]): Record<string, number> {
