...
export function estCritereSatisfaction(critere: ReponseSatisfaction['critere']): boolean {
  const type = String(critere?.type_reponse || '').toUpperCase();
  if (type === 'TEXTE' || type === 'QCM' || type === 'CASES') return false;
  if (type === 'NPS') return false;               // métrique dédiée
  if (type === 'CES') return false;                // (type explicite, par sécurité)
 ...
 */
export function noteSur5(r: ReponseSatisfaction | null | undefined): number | null {
  if (!r) return null;
...
export function notesSur5(reponses: ReponseSatisfaction[]): number[] {
  return reponses.map(noteSur5).filter((n): n is number => n !== null);
}

/** Moyenne de satisfaction sur 5, ou `null` si aucune réponse notable. */
export function moyenneSur5(reponses: ReponseSatisfaction[]): number | null {
