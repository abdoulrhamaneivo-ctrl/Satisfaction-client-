...
function exigerRoleGestion(context: any) {
  requireRole(context, ['DIRECTION', 'CHEF_AGENCE']);
}

function exigerJour(val: any): number {
  const n = Number(val);
...
function exigerHeures(debut: any, fin: any) {
  ...
  if (fin <= debut) throw new HttpError(400, "L'heure de fin doit être postérieure à l'heure de début.");
}

// ── Lecture de la grille ─────────────────────────────────────────────

export const getModelesHoraires = async (args: { id_agence: number }, context: any) => {
  requireAuth(context);
...
export const upsertModeleHoraire = async (args: any, context: any) => {
  requireAuth(context);
  await assertEntrepriseActive(context, context.entities);
...
export const deleteModeleHoraire = async (args: { id: number }, context: any) => {
  requireAuth(context);
  await assertEntrepriseActive(context, context.entities);
  exigerRoleGestion(context);
...
export const genererPlanning = async (
  args: { id_agence: number; date_debut: string; date_fin: string },
  context: any
) => {
  requireAuth(context);
  await assertEntrepriseActive(context, context.entities);
...
export const reconduirePlanning = async (
  args: { id_agence: number; date_source: string; date_cible: string },
  context: any
) => {
  requireAuth(context);
  await assertEntrepriseActive(context, context.entities);
...
export const suggererPlanning = async (args: { id_agence: number; date: string }, context: any) => {
  requireAuth(context);
  await assertEntrepriseActive(context, context.entities);
...
export const appliquerSuggestion = async (
  args: {
    id_agence: number;
    date: string;
