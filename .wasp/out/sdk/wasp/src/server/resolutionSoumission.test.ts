...
function qcmInverseOrdre(withProvenance = 'INFERRED'): any {
  return {
    id: 12,
    type_reponse: 'QCM',
    scoring_mode: null,
...
describe('QCM : identité, jamais position', () => {
  ...
  test('legacy texte : apparié par libellé (accents/casse), stampé MIGRATED', () => {
    const r = resoudreEntree(
      qcmInverseOrdre(), normaliserEntree({ critereId: 12, score: 1, texte: '  tres SATISFAIT ' }),
    );
    expect(r.score_officiel).toBe(5);
  ...
  test('legacy texte inconnu → 400 (pas de score inventé)', () => {
    expect(() =>
      resoudreEntree(qcmInverseOrdre(), normaliserEntree({ critereId: 12, score: 9, texte: 'Fantôme' })),
    ).toThrowError(HttpError);
  });

  test('optionId inconnue → 400', () => {
    expect(() =>
      resoudreEntree(qcmInverseOrdre(), normaliserEntree({ critereId: 12, optionId: 'zzz' })),
    ).toThrowError(HttpError);
  });
  ...
  test('option inactive → 400', () => {
    ...
    expect(() =>
      resoudreEntree(c, normaliserEntree({ critereId: 12, optionId: 'o-ins' })),
    ).toThrowError(HttpError);
  });

  test('sans optionId ni texte → 400', () => {
    expect(() =>
      resoudreEntree(qcmInverseOrdre(), normaliserEntree({ critereId: 12, score: 3 })),
    ).toThrowError(HttpError);
  });
});
...
describe('CASES : ids, pondéré, exclusivité', () => {
  ...
  test('legacy texte joint → mêmes ids, source MIGRATED', () => {
    const r = resoudreEntree(
      casCases(), normaliserEntree({ critereId: 15, texte: 'Attente longue • Information insuffisante' }),
    );
    expect(r.score_normalise).toBe(65);
    expect(r.score_source).toBe('MIGRATED');
  });
  ...
  test('« Aucun » + autre → 400', () => {
    expect(() =>
      resoudreEntree(casCases(), normaliserEntree({ critereId: 15, optionIds: ['opt_0', 'opt_1'] })),
    ).toThrowError(HttpError);
  });
...
describe('TEXTE : jamais noté', () => {
  test('verbatim long → NULL + NULL, texte conservé', () => {
    ...
    const r = resoudreEntree(
      c, normaliserEntree({ critereId: 14, score: 3, texte: 'Le personnel était très gentil.' }),
    );
