      ...
      <div className="flex items-center gap-3 rounded-2xl border border-border/80 bg-card p-3">
        ...
        >
          {ACTIONS_FILTRE.map((a) => (
            <option key={a} value={a}>{a === '' ? 'Toutes les actions' : ACTION_LABELS[a] ?? a}</option>
          ))}
        </select>
      </div>

      {isLoading ? (
        <div className="space-y-3" aria-busy="true" aria-label="Chargement du journal d'audit">
          {[0, 1, 2, 3].map((i) => (
        ...
        <div className="overflow-hidden rounded-2xl border border-border/80 bg-card">
          <div className="overflow-x-auto momentum-scroll scroll-fade-x" role="region" aria-label="Journal d'audit" tabIndex={0}>
            ...
            <tbody>
              {data.logs.map((l: {
                id: bigint; action: string; resource: string; resource_id: string | null;
                actor_role: string | null; ip: string | null; created_at: string | Date;
                acteur: { email: string | null; nom: string | null; prenom: string | null } | null;
              }) => (
                <tr key={String(l.id)} className="border-b border-border/50 last:border-0">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {l.acteur?.email && (
                      <span className="ml-2 hidden text-xs text-muted-foreground lg:inline">{l.acteur.email}</span>
                    )}
                  </td>
