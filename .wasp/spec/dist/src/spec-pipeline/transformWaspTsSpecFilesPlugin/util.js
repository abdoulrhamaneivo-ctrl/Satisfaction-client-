...
export function getTopLevelBindings(ast) {
    ...
    assert(topLevelScope, "Expected to find a scope for the top-level program node");
    const bindings = new Set(topLevelScope.defined);
    return bindings;
}
export function makeSafeName(desiredName, definedBindings) {
    let i = 0;
...
export function buildImportStatement(imports, source) {
    const specifiers = imports
        .map(([importName, localName]) => !localName || importName === localName
        ? importName
