...
import { Settings2, Search, RotateCcw, Archive, Copy, Trash2 } from 'lucide-react';
...
export const ConfigurationCriteresPage = () => {
    ...
    const activeIds = agenceCriteresIds || [];
          ...
          <div className="space-y-6">
            <MotionCard className="h-fit p-6 space-y-5">
              ...
              <form onSubmit={handleCreateCustom} className="space-y-4">
                ...
                {(typeReponse === 'QCM' || typeReponse === 'CASES') && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>
                    ...
                    <Input type="text" required value={optionsReponse} onChange={(e) => setOptionsReponse(e.target.value)} placeholder="Ex: Trop d'attente, Personnel absent, Autre"/>
                ...
                <div>
                  ...
                  <div className="space-y-2 rounded-md border border-input p-3 bg-background/50">
                    <RadioGroup value={selectedServiceIds.length === 0 ? 'NONE' : String(selectedServiceIds[0])} onValueChange={(v) => setSelectedServiceIds(v === 'NONE' ? [] : [Number(v)])} className="space-y-2">
                      <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer">
                        <RadioGroupItem value="NONE"/>
                        Non assignée
                      </label>
                      {services?.map((s) => (<label key={s.id} className="flex items-center gap-2 text-sm text-foreground cursor-pointer">
