 ...
 */
export const detecterAlertesSilence = async (_args, _context) => {
    ...
    for (const affectation of affectationsActives) {
        ...
        const joursOuverts = (guichet.agence.jours_ouvres || '')
            .split(',')
            .map((jour) => Number(jour.trim()))
            .filter(Number.isInteger);
        if (!joursOuverts.includes(jourOuvreActuel))
            continue;
        if (heureNow < guichet.agence.heure_ouverture || heureNow > guichet.agence.heure_fermeture)
            continue;
        ...
        const debutSurveillance = new Date(Math.max(debutJour.getTime() + ((heureOuverture * 60 + minuteOuverture) * 60_000), debutJour.getTime() + ((heureAffectation * 60 + minuteAffectation) * 60_000)));
        ...
        if (maintenant.getTime() - debutSurveillance.getTime() < 2 * 60 * 60 * 1000)
            continue;
        ...
        if (dernierAvis && dernierAvis >= debutSurveillance)
            continue;
    ...
    console.log(`[SILENCE] Job terminé — ${alertesCreees} alerte(s) créée(s), ${messagesEnvoyes} message(s) envoyé(s) (consolidés) sur ${affectationsActives.length} guichet(s) actifs`);
    return { alertesCreees, messagesEnvoyes };
};
//# sourceMappingURL=alerteSilence.js.map